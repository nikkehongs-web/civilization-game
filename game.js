...
const JOB_SKILL={'생명관찰자':'사육','생활경영자':'기록','농지운영자':'농업','씨앗선별자':'농업','수로구조사':'목공','공동부엌지기':'요리','동물돌봄이':'사육','도공':'도공','농부':'농업','목수':'목공','요리사':'요리','약초사':'약초','사육사':'사육','기록자':'기록','채집가':'채집'};
const RIVER_BASE_X=30,RIVER_WIDTH=14,RIVER_BANK_WIDTH=20;
function riverXAtZ(z){return RIVER_BASE_X+Math.sin(z/58)*5.5+Math.sin(z/21)*1.8}
function riverDistanceAt(x,z){return Math.abs(x-riverXAtZ(z))}
const LOC={center:new THREE.Vector3(0,0,0),field:new THREE.Vector3(-50,0,38),river:new THREE.Vector3(riverXAtZ(12),0,12),forest:new THREE.Vector3(-35,0,-24),stone:new THREE.Vector3(33,0,-23),workshop:new THREE.Vector3(10,0,-13),herbs:new THREE.Vector3(-7,0,28),pen:new THREE.Vector3(20,0,25),meeting:new THREE.Vector3(0,0,-4)};
...
function skillsFromOfficial(c){
 ...
 if(/사육|생명|동물/.test(f)){boost('사육',62);boost('기록',42)}
...
function officialResident(c,year){
 const age=Math.max(0,year-c.birth),stage=residentLifeStage(age),eraJob=(c.job==='동물돌봄이'?'생명관찰자':c.job),currentJob=age<12?'아이':age<16?'견습생':eraJob;
...
const INITIAL_LOCAL_IDS=new Set(WORLD_PEOPLE_SEED.filter(p=>p.local===1).map(p=>p.id));
if(INITIAL_LOCAL_IDS.size!==30)throw new Error(`감나무뜰 초기 인구 오류: ${INITIAL_LOCAL_IDS.size}/30`);
const MYEONGJA_DIED_BEFORE_WORLD_ZERO=true;
function isMyeongjaDeathTime(year=state?.year??0,day=state?.day??1){return false}

function officialEligible(c,year,day){
 if(!INITIAL_LOCAL_IDS.has(c.id))return false;
 if(c.id==='C0001')return !isMyeongjaDeathTime(year,day);
 return true
}
function initialOfficialResidents(year=0,day=1){
 return OFFICIAL_LOCAL_CATALOG.filter(c=>officialEligible(c,year,day)).map(c=>{const r=officialResident(c,year);r.homeCity='L001';return r})
}
...
const ACTION_META={
 ...
 farm:{icon:'🌱',label:'밭 돌보기'},cook:{icon:'🥣',label:'공동 취사'},animals:{icon:'🧬',label:'복실이·생명 관찰'},waterwork:{icon:'🛠',label:'물길 점검'},
...
const WORLD_START_POPULATION=300;
const POPULATION_MODEL_VERSION=1300;
const LOCAL_DETAIL_CAP=220;
const INITIAL_REGION_COMMUNITIES=[
 {cityId:'L001',country:'에르단 왕국',region:'아르케아 중앙대륙',population:30,label:'감나무뜰'},
 {cityId:'L019',country:'나엘 숲의회',region:'실바리온 수림권',population:39,label:'나엘 초기공동체'},
 {cityId:'L031',country:'카르딘 공방공화국',region:'카르딘 산맥권',population:39,label:'카르딘 초기공동체'},
 {cityId:'L043',country:'세라칸 부족회의',region:'세라칸 대초원',population:39,label:'세라칸 초기공동체'},
 {cityId:'L055',country:'네레이아 해양평의회',region:'네레이아 해권',population:39,label:'네레이아 초기공동체'},
 {cityId:'L067',country:'솔라크 수정왕정',region:'솔라크 사막권',population:38,label:'솔라크 초기공동체'},
 {cityId:'L079',country:'드라바스 화산연맹',region:'드라바스 화산군도',population:38,label:'드라바스 초기공동체'},
 {cityId:'L091',country:'루메라 부유연합',region:'루메라 부유제도',population:38,label:'루메라 초기공동체'}
];
...
function seedPopulationDistribution(){
 const out=Object.fromEntries(Object.keys(COUNTRY_META).map(n=>[n,0]));for(const s of INITIAL_REGION_COMMUNITIES)out[s.country]=s.population;return out
}
function initialPopulationDistribution(){return seedPopulationDistribution()}
function canonicalPopulationForYear(year=0){return Math.max(300,Math.round(300*Math.pow(1.032,Math.max(0,Math.min(260,year)))))}
...
function initialCountryState(){
 const out={},dist=initialPopulationDistribution();
 for(const n of Object.keys(COUNTRY_META)){
  const starter=INITIAL_COMMUNITY_BY_COUNTRY.get(n),pop=starter?.population||0,h=stableHash01(n);
  out[n]={population:pop,lastPopulation:pop,trackedPopulation:starter?WORLD_PEOPLE_SEED.filter(p=>p.c===n&&p.alive!==0).length:0,
   demography:demographicSnapshot(pop,n),cityPopulation:starter?{[starter.cityId]:pop}:{},lastWarEndYear:-999,warFatigue:0,
   region:COUNTRY_META[n].region,specialization:REGION_KNOWLEDGE_FOCUS[COUNTRY_META[n].region]||'생활기술',cities:COUNTRY_META[n].cities,settlements:starter?1:0,growth:.022+h*.015,
   aggression:Math.round(20+h*65),openness:Math.round(25+stableHash01(n+'o')*65),prosperity:Math.round(35+stableHash01(n+'p')*45),births:0,deaths:0,migration:0,
   children:starter?Math.round(pop*.24):0,adults:starter?Math.round(pop*.70):0,elders:starter?Math.max(1,Math.round(pop*.06)):0,households:starter?Math.max(1,Math.round(pop/4.3)):0,workers:starter?Math.round(pop*.48):0,
   food:starter?Math.round(52+stableHash01(n+'food')*22):0,tech:starter?Math.round(4+stableHash01(n+'tech')*8):0,infrastructure:starter?Math.round(4+stableHash01(n+'infra')*7):0,military:0,
   exploration:starter?Math.round(3+stableHash01(n+'exp')*6):0,stability:starter?Math.round(58+stableHash01(n+'stab')*24):0,knownCountries:starter?[n]:[],history:[],events:[],stage:starter?'초기 공동체':'미개척',
   economy:{grain:starter?42:0,wood:starter?34:0,stone:starter?22:0,metal:starter?5:0,livestock:0,cloth:0,fish:0},diplomacy:{},treaties:[],tradeRoutes:[],refugeesIn:0,refugeesOut:0,
   army:{mobilized:0,supply:100,morale:70,casualties:0,veterans:0},ecology:{deer:0,rabbit:0,boar:0,wolf:0,forest:72,soil:68,water:70,pressure:0},causal:[],active:!!starter,bornYear:starter?0:null,dissolvedYear:null,origin:n,
   capitalId:starter?.cityId||(WORLD_DATA.cities.find(x=>x.country===n)||{}).id||null,lastSettlementSplitYear:-999,
   politics:{form:'공동회의',leaderId:null,leaderSince:0,legitimacy:66,unrest:12,factions:{생활민:55,장인:45,교역민:25,무장대:8},history:[]}}
 }
 return out
}
...
function defaultDogLineage(){
 return{
   ...
   dogs:[{
     ...
     bornAbsDay:-1095,lifespanDays:999999999,parents:[],alive:true,role:'마을경비'
   }]
 }
}

function defaultClimateState(){return{season:'봄',weather:'맑음',temperature:14,wetness:50,lastWeek:-1,lastSeason:'봄'}}
...
function initialWorldSettlements(){return INITIAL_REGION_COMMUNITIES.map(x=>x.cityId)}
function cloneWorldPeopleSeed(){return WORLD_PEOPLE_SEED.map(p=>({...p,parents:[...(p.parents||[])]}))}
...
function defaultWorldState(){
 return{
  ...
  corePopulation:30,otherPopulation:270,populationModelVersion:POPULATION_MODEL_VERSION,totalPopulationModel:'eight-regions-300'
 }
}
function normalizeWorldState(){
 ...
 if((state.world.populationModelVersion||0)<POPULATION_MODEL_VERSION){
   const freshCountries=initialCountryState();state.world.countries=freshCountries;state.worldPeople=cloneWorldPeopleSeed();state.world.foundedCities=initialWorldSettlements();
   state.world.routes=[];state.world.expeditions=[];state.world.contactedCountries=['에르단 왕국'];state.world.knownRegions=['아르케아 중앙대륙'];state.world.externalWars=[];state.world.wars=[];
   state.world.populationModelVersion=POPULATION_MODEL_VERSION;state.world.totalPopulationModel='eight-regions-300';state.world.lastGlobalWarStartYear=-999
 }
 state.world.cityControl??=Object.fromEntries(WORLD_DATA.cities.map(c=>[c.id,c.country]));

 state.world.foundedCities??=[];
 for(const name of activeCountryNames()){
   const c=state.world.countries[name];if(!c||c.active===false)continue;
   let cap=c.capitalId?WORLD_CITY_BY_ID.get(c.capitalId):null;
   if(!cap||cityController(cap)!==name){
     cap=controlledCities(name)[0]||WORLD_DATA.cities.find(x=>x.country===name)||null;
     if(cap)c.capitalId=cap.id
   }
   if(cap&&!state.world.foundedCities.includes(cap.id))state.world.foundedCities.push(cap.id)
 }
for(const c of WORLD_DATA.cities)state.world.cityControl[c.id]??=c.country;
 ...
 for(const e of state.world.expeditions){
   e.memberIds??=[];e.phase??=(e.active?'outbound':'complete');
 ...
 for(const n of activeCountryNames())redistributeCityPopulation(n);
...
function travelSpeedKmDay(kind='land'){
 ...
 const mounted=state.bio?.engineeredLabor&&state.buildings?.pen&&state.tech?.사육기초?.open;
...
const RARITY_META=[
 {id:'common',name:'일반',mult:1.00,price:1.0},
 {id:'uncommon',name:'고급',mult:1.18,price:1.5},
 {id:'rare',name:'희귀',mult:1.42,price:2.2},
 {id:'epic',name:'영웅',mult:1.78,price:3.5},
 {id:'legendary',name:'전설',mult:2.25,price:5.5}
];
const MONSTER_RANKS={
 normal:{name:'일반',hp:1,atk:1,scale:1,gold:1,drop:.18},
 mutant:{name:'변이',hp:1.55,atk:1.25,scale:1.10,gold:1.7,drop:.36},
 elite:{name:'엘리트',hp:2.45,atk:1.55,scale:1.22,gold:2.7,drop:.72},
 prototype:{name:'프로토타입',hp:4.2,atk:2.05,scale:1.38,gold:5.2,drop:1}
};
const MONSTER_MUTATIONS=[
 {id:'regen',name:'재생조직',hp:1.18,atk:1,speed:1,interval:1},
 {id:'swift',name:'신경과속',hp:.92,atk:1.08,speed:1.28,interval:.84},
 {id:'armor',name:'골격경화',hp:1.28,atk:1.02,speed:.9,interval:1},
...
function defaultBioState(){return{score:0,stage:'복실이 관찰',artificialDog:false,livestock:false,engineeredLabor:false,militaryPrograms:false,firstOrc:false,dragonProject:false,lastMilestoneYear:-1}}
function bioStageLabel(){const b=state.bio||defaultBioState();return b.firstOrc?'신생종·마수 시대':b.militaryPrograms?'금지된 군사생명공학':b.engineeredLabor?'목적형 생명 설계':b.livestock?'인공가축 시대':b.artificialDog?'최초 인공동물':'복실이·인간·식물뿐'}
function updateBioScienceAnnual(){
 const b=state.bio??=defaultBioState(),shortage=Math.max(0,100-Math.min(100,state.worldPopulation/35)),bioLeader=state.world.countries['나엘 숲의회'];b.score+=7+avgSkill('기록')*.035+avgSkill('사육')*.045+(bioLeader?.tech||0)*.085+shortage*.045;
 if(state.year>=20&&!b.artificialDog&&b.score>=150){b.artificialDog=true;b.stage='최초 인공동물';state.tech.사육기초.open=true;state.tech.사육기초.p=100;addLog('story','인간이 처음 새로운 동물을 만들다','생명과학이 가장 빠르던 실바리온의 연구공동체가 인간·식물·복실이의 생명구조를 비교해 처음으로 완전한 비인간 인공생명체를 만들었다. 옛 기록의 이름을 빌려 사람들은 그것을 개라고 불렀다. 복실이의 복제가 아니라 인간이 설계한 새로운 생명이었다.','기록','사람이 너무 적었기에, 사람의 일을 대신할 생명을 만드는 연구가 시작됐다.')}
 if(state.year>=24&&b.artificialDog&&!b.livestock&&b.score>=205){b.livestock=true;b.stage='인공가축 시대';addLog('story','목축이 발명되다','식량·젖·운반을 위해 서로 다른 인공생물이 설계되기 시작했다. 이 세계의 목축은 야생동물을 길들인 것이 아니라 인간이 필요에 맞춰 생명을 만든 결과였다.','기록','인간이 적었기 때문에 한 사람이 하던 일을 생물과 기계가 나누기 시작했다.')}
 if(state.year>=31&&b.livestock&&!b.engineeredLabor&&b.score>=285){b.engineeredLabor=true;b.stage='목적형 생명 설계';addLog('story','필요한 생명을 설계하다','광산·건설·수송에 맞춘 생물 설계가 시작됐다. 사람 수가 적다는 약점이 기계화에 이어 생명공학을 다시 가속했다.','기록','말보다 오래 달리고 소보다 강한 생물을 만들 수 있다면?')}
 if(state.year>=41&&b.engineeredLabor&&!b.militaryPrograms&&b.score>=380){b.militaryPrograms=true;b.stage='금지된 군사생명공학';addLog('warn','인간이 넘지 말아야 할 선','국가 간 경쟁 속에서 재생의학과 노동생물 연구 일부가 군사용 실험으로 바뀌었다. 기록에서 사라지는 시설과 사람이 늘기 시작했다.','기록','병사 한 명을 키우는 데 수십 년이 걸린다는 사실이 가장 위험한 유혹이 됐다.')}
 if(state.year>=55&&!b.firstOrc){b.firstOrc=true;b.stage='신생종·마수 시대';addLog('warn','세계력 55년 · 첫 오크 조우','끊긴 교역로를 확인하러 간 정찰대에서 한 명만 돌아왔다. 그는 사람보다 큰 발자국과 낯선 도끼를 본 뒤 말했다. “사람처럼 생겼습니다. 아닙니다. 그런데 말을 했습니다.”','기록','그날 처음으로 감나무뜰은 오크를 만났다.')}
 b.stage=bioStageLabel()
}
...
function defaultRpgState(){
 return{
   ...
   skillPoints:0,skills:{strength:0,survival:0,critical:0,absorb:0},
   discoveries:[],experimentKills:{},totalLoot:0,lastDrop:null
 }
}

function fresh(){return{year:0,day:1,speed:1,running:true,weather:'맑음',climate:defaultClimateState(),resources:{food:22,water:28,wood:14,stone:8,labor:22},caps:{food:100,water:100,wood:100,stone:100,labor:60},buildings:{house:7,field:0,storage:0,workshop:0,herb:0,pen:0,meeting:0,well:0,kiln:0,kitchen:0,watch:0,loom:0},tech:{기록습관:{p:0,open:false},공동취사:{p:0,open:false},건조저장:{p:0,open:false},목공기초:{p:0,open:false},약초분류:{p:0,open:false},사육기초:{p:0,open:false},수로관리:{p:0,open:false},토기저장:{p:0,open:false},직조기초:{p:0,open:false},야간교대:{p:0,open:false}},residents:initialOfficialResidents(0,1),logs:[{seq:1,type:'story',time:'0년 1일',title:'사람만 존재하는 세계',text:'인간 300명 가운데 30명이 감나무 옛집 주변에서 시작했다. 숲에도 강에도 동물은 없었다. 인간과 식물, 그리고 아무도 만들지 않은 복실이만 존재했다.',speaker:'기록',quote:'누구도 명령하지 않았다. 복실이는 스스로 선택했다.'}],seq:1,flags:{firstField:false,firstHarvest:false,storage:false,illness:false,myeongjaDead:true,myeongjaDeathLogged:true},currentStorySeq:1,demography:{births:0,arrivals:0,children:0},civ:{level:0,levelName:'야영지',techUnlocked:0,builds:0,lastAnnual:null},eventMemory:{},worldPopulation:WORLD_START_POPULATION,world:defaultWorldState(),worldPeople:cloneWorldPeopleSeed(),worldPersonSeq:1000000,localMap:{level:0,revealedRadius:90,lastExpansionYear:-1},animalStats:{wild:0,domestic:0,care:0},trajectory:defaultTrajectory(),conflict:{animalRaids:0,warBattles:0,lastAnimalRaidDay:-999,lastBattleDay:-999,foodLost:0,wounded:0,raidersDefeated:0,animalsRepelled:0},companion:{bokshil:{x:1.2,z:4.5,active:true}},dogLineage:defaultDogLineage(),deceased:[{id:'C0001',name:'이명자',year:-1,day:0,cause:'현실에서 사망 · 테스트 서버 시작 이전'}],bio:defaultBioState(),rpg:defaultRpgState(),monsterEvolution:defaultMonsterEvolution(),observerTransport:{mode:'walk'},player:{x:3,z:3,level:1,exp:0,nextExp:100,hp:100,maxHp:100,attack:14,kills:0,deaths:0,dead:false,respawnAt:0,awakened:false,autoHunt:false}}};
let state;try{state=JSON.parse(localStorage.getItem(KEY))||fresh()}catch{state=fresh()}

function mergeOfficialResidents(){
 const existing=[...(state.residents||[])],old=new Map(existing.map(r=>[r.id,r])),deadIds=new Set((state.deceased||[]).map(d=>d.id));
 const official=OFFICIAL_LOCAL_CATALOG.filter(c=>officialEligible(c,state.year||0,state.day||1)&&!deadIds.has(c.id)).map(c=>{
  const base=officialResident(c,state.year||0),prev=old.get(c.id);if(!prev){base.homeCity='L001';return base}
  const m={...base,...prev};for(const k of ['name','gender','family','generation','originJob','field','potential','potentialGrade','growthType','value','fear','flaw','hiddenTrait','surfaceTrait','trueDesire','habit','speech','stressResponse','affectionStyle','achievementSeed','changeArc','personality','note'])m[k]=base[k];
  m.age=Math.max(0,(state.year||0)-c.birth);m.lifeStage=residentLifeStage(m.age);m.careerSeed=base.careerSeed||c.job;m.homeCity??='L001';m.skills={...base.skills,...(prev.skills||{})};return initResidentBrain(m)
 });
 const dynamic=existing.filter(r=>!OFFICIAL_BY_ID.has(r.id)&&!deadIds.has(r.id)).map(r=>{r.homeCity??='L001';return initResidentBrain(r)});
 state.residents=[...official,...dynamic]
}
function migrate(){
...
state.flags??={};state.storyCooldowns??={};state.schemaVersion=102;state.climate??=defaultClimateState();
...
for(const d of state.dogLineage.dogs){d.alive??=true;d.parents??=[];d.generation??=0;d.bornAbsDay??=-1095;d.lifespanDays??=(d.id==='DOG-BOKSHIL'?999999999:4745)}
state.deceased??=[];
...
state.deceased.unshift({id:'C0001',name:'이명자',year:-1,day:0,cause:'현실에서 사망 · 테스트 서버 시작 이전'});
state.residents=state.residents.filter(r=>r.id!=='C0001');
...
state.monsterEvolution??=defaultMonsterEvolution();state.bio??=defaultBioState();state.world.observerLocation??=defaultObserverLocation();
...
state.rpg.equipment??={weapon:null,armor:null,accessory:null};state.rpg.inventory??=[];state.rpg.skills??={strength:0,survival:0,critical:0,absorb:0};
...
if(!state.flags.civEngineV2){state.flags.civEngineV2=true}
state.demography.children=state.residents.filter(r=>r.age<16).length;
state.demography.births??=0;
state.demography.arrivals=0;
recalculateCaps();
...
function seasonIndex(){return state.day<92?0:state.day<183?1:state.day<274?2:3}
function regionClimateProfile(region){
 if(/사막/.test(region))return{dry:.78,wet:.15,cold:.08};
 if(/해권|군도/.test(region))return{dry:.18,wet:.72,cold:.18};
 if(/산맥/.test(region))return{dry:.28,wet:.42,cold:.65};
 if(/수림/.test(region))return{dry:.18,wet:.68,cold:.28};
 if(/대초원/.test(region))return{dry:.48,wet:.27,cold:.38};
 return{dry:.30,wet:.45,cold:.28}
}
function deterministicRegionWeather(region){
 const s=season(),week=Math.floor((state.day-1)/7),p=regionClimateProfile(region),r=stableHash01(`${region}|${state.year}|${week}|weather`);
 let opts;
 if(s==='봄')opts=p.wet>.55?['맑음','흐림','봄비','봄비','강풍']:['맑음','맑음','흐림','봄비','강풍'];
 else if(s==='여름')opts=p.dry>.6?['맑음','폭염','폭염','강풍','소나기']:p.wet>.6?['흐림','소나기','폭우','맑음','소나기']:['맑음','소나기','폭염','흐림','맑음'];
 else if(s==='가을')opts=p.wet>.6?['맑음','흐림','가을비','강풍','가을비']:['맑음','맑음','흐림','강풍','가을비'];
 else opts=p.cold>.5?['눈','폭설','흐림','맑음','눈']:['맑음','흐림','찬바람','눈','맑음'];
 return opts[Math.min(opts.length-1,Math.floor(r*opts.length))]
}
function climateTemperature(region){
 const s=season(),base={봄:14,여름:27,가을:16,겨울:2}[s],p=regionClimateProfile(region);let t=base;
 if(/사막/.test(region))t+=s==='여름'?8:4;if(/산맥/.test(region))t-=7;if(/해권|군도/.test(region))t+=s==='겨울'?4:-2;
 const w=deterministicRegionWeather(region);if(w==='폭염')t+=7;if(w==='폭설'||w==='눈')t-=4;return Math.round(t)
}
function updateClimateDaily(force=false){
 state.climate??=defaultClimateState();state.world.climateByRegion??={};const week=Math.floor((state.day-1)/7);
 if(!force&&state.climate.lastWeek===week&&state.climate.season===season())return;
 for(const r of WORLD_DATA.regions){const weather=deterministicRegionWeather(r.name),temp=climateTemperature(r.name);state.world.climateByRegion[r.name]={season:season(),weather,temperature:temp,week,year:state.year}}
 const local=state.world.climateByRegion['아르케아 중앙대륙'];state.climate={...state.climate,...local,lastWeek:week,lastSeason:season()};state.weather=local.weather;
 if(typeof applySeasonVisuals==='function')applySeasonVisuals()
}
function countryClimateEvent(name,c){
 if(!c?.active)return;const r=stableHash01(`${name}|${state.year}|climate`);if(r>.24)return;
 const profile=regionClimateProfile(c.region),event=r<.045&&profile.wet>.45?'홍수':r<.09&&profile.dry>.35?'가뭄':r<.135&&profile.cold>.35?'폭설':r<.18?'폭염':'풍년';
 if(event==='풍년'){c.food=clamp(c.food+9,0,100);c.prosperity=clamp(c.prosperity+3,0,100);c.economy.grain=clamp(c.economy.grain+12,0,180)}
 else{c.food=clamp(c.food-7,0,100);c.stability=clamp(c.stability-3,0,100);c.economy.grain=clamp(c.economy.grain-8,0,180);if(event==='홍수')c.infrastructure=clamp(c.infrastructure-1.8,0,100)}
 pushCountryCause(name,'기후',`${event} · 식량/생활 조건 변화`,2)
}
function weatherFarmFactor(){const w=state.weather,s=season();let f=s==='겨울'?.35:s==='봄'?1.05:s==='여름'?1.15:1.0;if(w==='봄비'||w==='소나기'||w==='가을비')f*=1.12;if(w==='폭우'||w==='폭설'||w==='가뭄'||w==='폭염')f*=.62;return f}
...
// ---------- THREE WORLD ----------
const scene=new THREE.Scene();scene.background=new THREE.Color(0x94a4a1);scene.fog=new THREE.FogExp2(0xaab0a1,.0028);
const gameRoot=$('game');
...
renderer.shadowMap.enabled=true;renderer.shadowMap.type=THREE.PCFSoftShadowMap;renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.08;
...
resizeWorld(true);
scene.add(new THREE.HemisphereLight(0xdff2df,0x5a4938,2.1));const sun=new THREE.DirectionalLight(0xffe7b4,2.5);sun.position.set(-35,55,-30);sun.castShadow=true;sun.shadow.mapSize.set(IS_MOBILE?1024:2048,IS_MOBILE?1024:2048);sun.shadow.camera.left=-80;sun.shadow.camera.right=80;sun.shadow.camera.top=80;sun.shadow.camera.bottom=-80;scene.add(sun);
...
const LOCAL_LANDMARKS=[
 ...
 {name:'감나무 과수원',icon:'🟠',p:new THREE.Vector3(22,0,18),color:0xf0a34c},
...
function updateLocalMapExpansion(announce=true){
 ...
 for(const t of frontierTiles)t.g.visible=false;frontierGroup.visible=false;updateLandmarkVisibility?.();
...
function tree(x,z,s=.9){const g=new THREE.Group(),tr=new THREE.Mesh(new THREE.CylinderGeometry(.18,.3,2.5,7),new THREE.MeshStandardMaterial({color:0x57402d,roughness:1})),c1=new THREE.Mesh(new THREE.ConeGeometry(1.25,2.8,8),new THREE.MeshStandardMaterial({color:0x557d58,roughness:1,transparent:true,opacity:.74})),c2=new THREE.Mesh(new THREE.ConeGeometry(.95,2.2,8),new THREE.MeshStandardMaterial({color:0x688f67,roughness:1,transparent:true,opacity:.70}));tr.position.y=1.25;c1.position.y=3;c2.position.y=4;[tr,c1,c2].forEach(m=>m.castShadow=true);g.add(tr,c1,c2);g.position.set(x,0,z);g.scale.setScalar(s);scene.add(g);return g}
function persimmonTree(x,z,s=1){
 const g=new THREE.Group();
 const trunk=new THREE.Mesh(new THREE.CylinderGeometry(.17,.28,2.7,7),new THREE.MeshStandardMaterial({color:0x6a482f,roughness:1}));
 const crown1=new THREE.Mesh(new THREE.SphereGeometry(1.15,8,7),new THREE.MeshStandardMaterial({color:0x4f7b3f,roughness:1}));
 const crown2=new THREE.Mesh(new THREE.SphereGeometry(.92,8,7),new THREE.MeshStandardMaterial({color:0x5c8748,roughness:1}));
 trunk.position.y=1.35;crown1.position.set(-.18,3.0,.04);crown2.position.set(.45,3.35,-.12);
 [trunk,crown1,crown2].forEach(m=>m.castShadow=true);g.add(trunk,crown1,crown2);
 const fruitMat=new THREE.MeshStandardMaterial({color:0xe67f24,roughness:.75});
 const fruitPos=[[-.45,2.45,.28],[.38,2.58,.42],[.2,3.02,-.35],[-.18,3.12,-.1],[.55,2.92,.05],[-.55,2.86,-.28]];
 for(const [fx,fy,fz] of fruitPos){const f=new THREE.Mesh(new THREE.SphereGeometry(.11,8,8),fruitMat);f.position.set(fx,fy,fz);g.add(f)}
 g.position.set(x,0,z);g.scale.setScalar(s);scene.add(g);return g
}
for(let i=0;i<64;i++){const a=Math.random()*Math.PI*2,r=48+Math.random()*58;tree(Math.cos(a)*r+(Math.random()-.5)*10,Math.sin(a)*r*.75+(Math.random()-.5)*10,.6+Math.random()*.65)}
const persimmonOrchard=[[-24,10,.92],[-16,12,.98],[-8,10,.9],[1,12,.96],[10,10,1.02],[19,12,.94],[28,9,.9],[-20,19,.95],[-12,21,1.02],[-4,18,.92],[5,21,.98],[14,18,1.05],[23,21,.94],[31,17,.9],[-16,28,.9],[-7,29,.94],[3,28,.9],[13,29,.98],[23,28,.92]];
persimmonOrchard.forEach(([x,z,s])=>persimmonTree(x,z,s));
const orchardLabel=makeFloatingNameSprite('🟠 감나무 과수원');orchardLabel.position.set(22,3.6,18);orchardLabel.scale.set(3.25,.7,1);scene.add(orchardLabel)
// river is rendered from the same canonical riverXAtZ() data used by the minimap.
// It is streamed chunk-by-chunk below so the water continues beyond the original village map.
// paths
...
const riverWaterMaterial=new THREE.MeshStandardMaterial({
 color:0x6b9aaa,roughness:.26,metalness:.04,transparent:true,opacity:.93,
 emissive:0x17323a,emissiveIntensity:.08
});
const riverBankMaterial=new THREE.MeshStandardMaterial({color:0x71805e,roughness:1});
function riverSegmentMesh(width,a,b,material,y){
 const d=b.clone().sub(a),len=Math.max(.1,d.length());
 const m=new THREE.Mesh(new THREE.PlaneGeometry(width,len+1.2),material);
 m.rotation.x=-Math.PI/2;m.rotation.z=-Math.atan2(d.x,d.z);
 m.position.copy(a.clone().add(b).multiplyScalar(.5));m.position.y=y;
 m.receiveShadow=true;return m
}
function addCanonicalRiverToChunk(group,cx,cz){
 const ox=cx*STREAM_CHUNK,oz=cz*STREAM_CHUNK,half=STREAM_CHUNK/2;
 const z0=oz-half-8,z1=oz+half+8,step=12;
 for(let z=z0;z<z1;z+=step){
   const za=z,zb=Math.min(z+step,z1),xa=riverXAtZ(za),xb=riverXAtZ(zb);
   // Only chunks crossed by the canonical river receive river geometry.
   const minX=Math.min(xa,xb)-RIVER_BANK_WIDTH*.6,maxX=Math.max(xa,xb)+RIVER_BANK_WIDTH*.6;
   if(maxX<ox-half-2||minX>ox+half+2)continue;
   const a=new THREE.Vector3(xa-ox,0,za-oz),b=new THREE.Vector3(xb-ox,0,zb-oz);
   group.add(riverSegmentMesh(RIVER_BANK_WIDTH,a,b,riverBankMaterial,.005));
   group.add(riverSegmentMesh(RIVER_WIDTH,a,b,riverWaterMaterial,.035))
 }
}

const STREAM_BIOME_HEX={forest:0x83956f,lakewood:0x82956f,hills:0x858d70,grassland:0x909a70,wetland:0x7f947d,meadow:0x96a176};
function seasonTerrainHex(base){const col=new THREE.Color(base),s=season();if(s==='봄')col.lerp(new THREE.Color(0x79a866),.18);else if(s==='가을')col.lerp(new THREE.Color(0xa27c45),.34);else if(s==='겨울')col.lerp(new THREE.Color(0xc7ccc0),.48);return col.getHex()}
function buildStreamChunk(cx,cz){
 const key=chunkKey(cx,cz),g=new THREE.Group(),biome=chunkBiome(cx,cz);
 const inHandcraftedZone=Math.abs(cx)<=1&&Math.abs(cz)<=1;
 let plane=null;
 if(!inHandcraftedZone){
  plane=new THREE.Mesh(new THREE.PlaneGeometry(STREAM_CHUNK+2,STREAM_CHUNK+2),new THREE.MeshStandardMaterial({color:seasonTerrainHex(STREAM_BIOME_HEX[biome]||0x81905f),roughness:1}));
 ...
 addCanonicalRiverToChunk(g,cx,cz);

 // Natural decoration. Handcrafted village chunks keep their own terrain so no overlapping sheet appears.
 const central=Math.abs(cx)<=1&&Math.abs(cz)<=1;
 const count=central?0:(IS_MOBILE?8:13);
 for(let i=0;i<count;i++){
   const x=(chunkSeed(cx,cz,i*4)-.5)*(STREAM_CHUNK-14);
   const z=(chunkSeed(cx,cz,i*4+1)-.5)*(STREAM_CHUNK-14);
   const globalX=ox+x,globalZ=oz+z;
   if(riverDistanceAt(globalX,globalZ)<RIVER_BANK_WIDTH*.62)continue;
   const r=chunkSeed(cx,cz,i*4+2),s=.6+chunkSeed(cx,cz,i*4+3)*.75;
   if(biome==='forest'||biome==='lakewood'){
     if(r<.48)streamTree(g,x,z,s*.8);else if(r<.75)streamBush(g,x,z,s);else streamRock(g,x,z,s)
   }else if(biome==='hills'){
 ...
 if(!central&&((biome==='lakewood'&&waterChance<.52)||(biome==='wetland'&&waterChance<.42)||(waterChance<.055))){
   const px=(chunkSeed(cx,cz,779)-.5)*55,pz=(chunkSeed(cx,cz,780)-.5)*55;
   if(riverDistanceAt(ox+px,oz+pz)>RIVER_BANK_WIDTH+18){
     ...
     pond.rotation.x=-Math.PI/2;pond.position.set(px,.02,pz);pond.scale.y=.68;g.add(pond)
   }
 }
...
function removeStreamChunk(key){
 ...
 if(c.plane){const wi=walkableSurfaces.indexOf(c.plane);if(wi>=0)walkableSurfaces.splice(wi,1);}
...
function applySeasonVisuals(){
 const s=season(),w=state.weather||'맑음',bg={봄:0xa9b8ad,여름:0x9eafa9,가을:0xaa9f8b,겨울:0xc2c8c6}[s],fog=['폭우','폭설','흐림'].includes(w)?.0055:.0028;scene.background.setHex(bg);scene.fog.color.setHex(bg);scene.fog.density=fog;
 ground.material.color.setHex(seasonTerrainHex(0x7e895d));for(const t of frontierTiles)t.plane.material.color.setHex(seasonTerrainHex([0x748258,0x687957,0x78845f,0x657553,0x7f895d,0x6f8059][t.index%6]));for(const c of streamChunks.values())if(c.plane)c.plane.material.color.setHex(seasonTerrainHex(STREAM_BIOME_HEX[c.biome]||0x81905f));sun.intensity=w==='폭우'||w==='폭설'?1.25:w==='흐림'?1.7:2.5
}
...
function residentStable01(r,salt=''){return stableHash01(`${r.family||r.id}|${r.id}|${salt}`)}
function residentHomePosition(r){
 const hc=Math.max(2,state.buildings.house||Math.ceil(state.residents.length/4.6)),idx=Math.floor(stableHash01(r.family||r.id)*hc),s=houseSpot(idx);
 const a=residentStable01(r,'home-a')*Math.PI*2,rr=.7+residentStable01(r,'home-r')*1.5;
 return new THREE.Vector3(s[0]+Math.cos(a)*rr,0,s[1]+Math.sin(a)*rr)
}
function spreadAround(r,base,minR,maxR,salt='work'){
 const a=residentStable01(r,`${salt}-a`)*Math.PI*2,rr=minR+residentStable01(r,`${salt}-r`)*(maxR-minR);
 return base.clone().add(new THREE.Vector3(Math.cos(a)*rr,0,Math.sin(a)*rr))
}
function distributedRiverTarget(r,salt='river'){
 const z=LOC.river.z+(residentStable01(r,`${salt}-z`)-.5)*82,side=residentStable01(r,`${salt}-side`)<.5?-1:1;
 return new THREE.Vector3(riverXAtZ(z)+side*(RIVER_WIDTH*.55+1.4+residentStable01(r,`${salt}-bank`)*2.5),0,z)
}
...
function localSettlementPopulation(){return Math.max(0,cityPopulationCount('L001','에르단 왕국')||state.residents.filter(r=>(r.homeCity||'L001')==='L001').length)}
...
function rebuildFrontierVillages(){while(frontierVillageGroup.children.length)frontierVillageGroup.remove(frontierVillageGroup.children.pop());frontierVillageSignature='canon-126-only'}
...
const HUMAN_WORLD_SCALE=0.38;
const REGION_HUMAN_SCALE=0.38;
const ANIMAL_WORLD_SCALE=0.48;
const DOG_WORLD_SCALE=0.30;
...
function populateWildAnimals(){state.animalStats.wild=0}
function ensureDomesticAnimals(){
 if(!state.bio?.livestock||!state.buildings.pen)return;
 const existing=animals.filter(a=>a.userData.domestic).length,desired=6;if(existing>=desired)return;
...
function startAnimalRaid(kind='crop'){return false}
...
function maybeConflictsDaily(){
 ...
 if(war&&state.year>=35&&battleGap>38&&Math.random()<.010)startWarRaid(war.country)
}
...
function createObserverTransportModel(){
 const root=new THREE.Group(),models={},mat=(c,m=0)=>new THREE.MeshStandardMaterial({color:c,roughness:m?.45:.82,metalness:m}),dark=mat(0x2f3434),steel=mat(0xb7c1c4,.55),blue=mat(0x405c6b,.2),glass=new THREE.MeshStandardMaterial({color:0x8fb5c0,roughness:.25,metalness:.1,transparent:true,opacity:.72});
 const rover=new THREE.Group(),body=new THREE.Mesh(new THREE.BoxGeometry(1.9,.42,.92),blue);body.position.y=.46;rover.add(body);const cabin=new THREE.Mesh(new THREE.BoxGeometry(1.05,.46,.76),glass);cabin.position.set(-.08,.82,0);rover.add(cabin);for(const x of[-.67,.67])for(const z of[-.42,.42]){const w=new THREE.Mesh(new THREE.CylinderGeometry(.14,.14,.10,12),dark);w.rotation.x=Math.PI/2;w.position.set(x,.25,z);rover.add(w)}rover.scale.setScalar(.62);models.rover=rover;root.add(rover);
 const plane=new THREE.Group(),fuse=new THREE.Mesh(new THREE.CapsuleGeometry(.18,1.7,5,10),steel);fuse.rotation.z=Math.PI/2;fuse.position.y=.68;plane.add(fuse);const wing=new THREE.Mesh(new THREE.BoxGeometry(.85,.06,2.7),blue);wing.position.y=.69;plane.add(wing);const tail=new THREE.Mesh(new THREE.BoxGeometry(.42,.05,1.05),blue);tail.position.set(-.82,.74,0);plane.add(tail);const fin=new THREE.Mesh(new THREE.BoxGeometry(.38,.55,.05),blue);fin.position.set(-.82,.99,0);plane.add(fin);const nose=new THREE.Mesh(new THREE.ConeGeometry(.18,.44,12),steel);nose.rotation.z=-Math.PI/2;nose.position.set(1.06,.68,0);plane.add(nose);plane.scale.setScalar(.84);models.plane=plane;root.add(plane);
 for(const g of Object.values(models))g.visible=false;root.userData.models=models;root.visible=false;scene.add(root);return root
}
const OBSERVER_TRANSPORTS={walk:{name:'도보',icon:'🚶',speed:1},rover:{name:'관찰차',icon:'🚙',speed:30},plane:{name:'관찰 비행기',icon:'✈️',speed:180}};
function unlockedObserverTransports(){return Object.keys(OBSERVER_TRANSPORTS)}
function observerTransportMultiplier(){return OBSERVER_TRANSPORTS[state.observerTransport?.mode]?.speed||1}
function updateTransportVisual(){const mode=state.observerTransport?.mode||'walk',t=OBSERVER_TRANSPORTS[mode]||OBSERVER_TRANSPORTS.walk,on=mode!=='walk',models=observerTransport?.userData?.models||{};for(const[k,g]of Object.entries(models))g.visible=on&&k===mode&&!regionViewActive;if(observerTransport)observerTransport.visible=on&&!regionViewActive;if(regionObserver?.userData?.transportVisual){regionObserver.userData.transportVisual.visible=on&&regionViewActive;regionObserver.userData.transportVisual.scale.setScalar(mode==='plane'?1.45:1)}$('transportBtn').textContent=t.icon;$('transportBtn')?.classList.toggle('active',on);$('transportHud')?.classList.toggle('hidden',!on);if(on)$('transportHud').textContent=`${t.icon} ${t.name} · ${t.speed}× · 관찰자 전용`}
function setObserverTransport(mode){if(!OBSERVER_TRANSPORTS[mode])mode='walk';state.observerTransport.mode=mode;updateTransportVisual();const t=OBSERVER_TRANSPORTS[mode];showEvent(`${t.icon} ${t.name} · ${t.speed}배`)}
function toggleObserverTransport(){const modes=unlockedObserverTransports(),cur=state.observerTransport?.mode||'walk',i=Math.max(0,modes.indexOf(cur));setObserverTransport(modes[(i+1)%modes.length])}

function createPlayerModel(){
 const g=new THREE.Group(),skin=new THREE.MeshStandardMaterial({color:0xd3a07c,roughness:.9}),cloth=new THREE.MeshStandardMaterial({color:0x43584f,roughness:1}),dark=new THREE.MeshStandardMaterial({color:0x303735,roughness:1}),metal=new THREE.MeshStandardMaterial({color:0xaeb8bb,roughness:.45,metalness:.45});
 const torso=new THREE.Mesh(new THREE.CylinderGeometry(.27,.32,.76,8),cloth);torso.position.y=1.25;g.add(torso);const head=new THREE.Mesh(new THREE.SphereGeometry(.21,10,8),skin);head.position.y=1.82;g.add(head);
 const limbs={};for(const[name,x,y,h,mat]of[['la',-.34,1.48,.60,skin],['ra',.34,1.48,.60,skin],['ll',-.14,.78,.70,dark],['rl',.14,.78,.70,dark]]){const p=new THREE.Group();p.position.set(x,y,0);const m=new THREE.Mesh(new THREE.CylinderGeometry(.055,.065,h,6),mat);m.position.y=-h/2;p.add(m);g.add(p);limbs[name]=p}
 const sword=new THREE.Group(),blade=new THREE.Mesh(new THREE.BoxGeometry(.045,.58,.025),metal);blade.position.y=-.34;sword.add(blade);sword.position.set(0,-.42,.03);limbs.ra.add(sword);sword.visible=false;
 const ring=new THREE.Mesh(new THREE.RingGeometry(.43,.52,28),new THREE.MeshBasicMaterial({color:0xe8d07f,transparent:true,opacity:.76,side:THREE.DoubleSide}));ring.rotation.x=-Math.PI/2;ring.position.y=.02;g.add(ring);
 g.position.set(state.player.x||3,0,state.player.z||3);g.scale.setScalar(HUMAN_WORLD_SCALE);g.traverse(o=>{if(o.isMesh){o.castShadow=true;o.receiveShadow=true}});
 g.userData={limbs,parts:{torso,head},sword,target:new THREE.Vector3(g.position.x,0,g.position.z),moving:false,attackCooldown:0,attackAnim:0,fsm:new StateMachine(PlayerStates.IDLE)};scene.add(g);return g
}
const player=createPlayerModel();
const observerTransport=createObserverTransportModel();
const playerHereLabel=makeFloatingNameSprite('▼ 나');playerHereLabel.position.set(0,3.78,0);playerHereLabel.scale.set(2.55,.67,1);player.add(playerHereLabel);
streamTerrainAroundPlayer(true);updateClimateDaily(true);applySeasonVisuals();
...
function createBokshilModel(){
 const g=new THREE.Group();
 const fur=new THREE.MeshStandardMaterial({color:0xd4bf9f,roughness:1});
 const cream=new THREE.MeshStandardMaterial({color:0xf0e2c6,roughness:1});
 const dark=new THREE.MeshStandardMaterial({color:0x47352a,roughness:1});
 const collar=new THREE.MeshStandardMaterial({color:0x8a5e48,roughness:.8});
 const body=new THREE.Mesh(new THREE.CapsuleGeometry(.26,.62,4,8),fur);body.rotation.z=Math.PI/2;body.position.y=.53;body.scale.set(1.22,.72,1.52);g.add(body);
 const chest=new THREE.Mesh(new THREE.SphereGeometry(.22,10,8),cream);chest.scale.set(.9,1.05,.7);chest.position.set(0,.55,.42);g.add(chest);
 const head=new THREE.Mesh(new THREE.SphereGeometry(.28,12,9),fur);head.scale.set(.88,.84,1.04);head.position.set(0,.82,.58);g.add(head);
 const muzzle=new THREE.Mesh(new THREE.SphereGeometry(.15,10,8),cream);muzzle.scale.set(1.05,.72,.86);muzzle.position.set(0,.75,.82);g.add(muzzle);
 const nose=new THREE.Mesh(new THREE.SphereGeometry(.05,8,6),dark);nose.position.set(0,.78,.95);g.add(nose);
 for(const x of[-.1,.1]){const eye=new THREE.Mesh(new THREE.SphereGeometry(.028,6,5),dark);eye.position.set(x,.9,.8);g.add(eye)}
 for(const x of[-.18,.18]){const ear=new THREE.Mesh(new THREE.ConeGeometry(.085,.24,6),fur);ear.position.set(x,1.05,.46);ear.rotation.z=x<0?.42:-.42;ear.rotation.x=.18;g.add(ear)}
 const legs=[];
 for(const [x,z] of [[-.18,.28],[.18,.28],[-.18,-.28],[.18,-.28]]){
  const p=new THREE.Group();p.position.set(x,.34,z);
  const upper=new THREE.Mesh(new THREE.CylinderGeometry(.048,.055,.18,6),fur);upper.position.y=-.08;p.add(upper);
  const lower=new THREE.Mesh(new THREE.CylinderGeometry(.04,.045,.2,6),cream);lower.position.y=-.27;p.add(lower);
  g.add(p);legs.push(p)
 }
 const tailPivot=new THREE.Group();tailPivot.position.set(0,.74,-.48);g.add(tailPivot);
 const tail=new THREE.Mesh(new THREE.TorusGeometry(.19,.03,6,14,Math.PI*1.15),fur);tail.rotation.x=.55;tail.rotation.z=Math.PI*.2;tail.position.set(0,.14,-.08);tailPivot.add(tail);
 const neck=new THREE.Mesh(new THREE.TorusGeometry(.2,.026,6,16),collar);neck.rotation.x=Math.PI/2;neck.position.set(0,.69,.28);g.add(neck);
 const name=makeFloatingNameSprite('복실이');name.position.set(0,1.28,0);name.scale.set(2.1,.54,1);g.add(name);
 g.position.set(state.companion.bokshil.x,0,state.companion.bokshil.z);
 g.scale.setScalar(.88*DOG_WORLD_SCALE);
...
function makeLineageDogModel(dog){
 ...
 const label=makeFloatingNameSprite(`${dog.name} · ${dog.generation||0}세대`);label.position.set(0,1.08,0);label.scale.set(2.2,.5,1);g.add(label);
 const scale=(pup?clamp(.42+dogAgeDays(dog)/365*.42,.42,.84):(.78+Math.min(.06,gen*.012)))*DOG_WORLD_SCALE;
...
function maybeDogOutsiderArrival(){return false}
function maybeDogLitter(){return false}
function processDogLineageDaily(){
 const bok=state.dogLineage.dogs.find(d=>d.id==='DOG-BOKSHIL');if(bok){bok.alive=true;bok.lifespanDays=999999999;state.companion.bokshil.active=true}
 state.dogLineage.dogs=state.dogLineage.dogs.filter(d=>d.id==='DOG-BOKSHIL');syncDogLineageVisuals()
}
...
function renderDogLineage(){
 const b=state.bio??defaultBioState(),steps=[['복실이','아무도 만들지 않은 유일한 비인간 동물',true],['비침습 생명관찰','털·침·자연 탈락 세포·행동 기록',true],['최초 인공 개','인간이 설계한 첫 비인간 생명',b.artificialDog],['인공가축','식량·젖·운반 목적 생물',b.livestock],['목적형 합성생물','광산·건설·수송·구조용 생물',b.engineeredLabor],['군사 생명공학','병력 부족을 대체하려는 금지 연구',b.militaryPrograms],['오크·마수·신생종','실험체가 독립 생명권으로 등장',b.firstOrc]];
 $('dogLineageSummary').textContent=`복실이는 생물학적 조상이 아니라 인공생명과학의 출발점 · 현재 ${bioStageLabel()}`;
 $('dogLineageFounder').innerHTML=`<b>★ 복실이 · 최초의 비인간 생명 기준점</b><span>복제하지 않음 · 강제실험/해부 금지 · 생존 상태 유지 · 연구점수 ${Math.floor(b.score||0)}</span>`;
 $('dogLineageTree').innerHTML=`<section class="dog-lineage-generation"><div><h3>생명과학 계통</h3><i class="gen-line"></i></div><div class="dog-generation-grid">${steps.map(([n,d,open],i)=>`<article class="dog-card ${open?'alive':'dead'}"><div class="dog-title"><b>${open?'●':'○'} ${n}</b><em>${i}단계</em></div><p>${open?d:'아직 도달하지 않은 연구 단계'}</p></article>`).join('')}</div></section>`
}
function openDogLineage(){
 renderDogLineage();$('dogLineagePanel').classList.remove('hidden')
}

function lineageThreat(){if(state.year<55)return null;
...
function rarityMeta(id){return RARITY_META.find(r=>r.id===id)||RARITY_META[0]}
function equipmentItems(){return Object.values(state.rpg.equipment||{}).filter(Boolean)}
function playerCombatStats(){
 const p=state.player,r=state.rpg,gear=equipmentItems(),skills=r.skills||{};
 const gearAttack=gear.reduce((s,i)=>s+(i.attack||0),0),gearDefense=gear.reduce((s,i)=>s+(i.defense||0),0),gearHp=gear.reduce((s,i)=>s+(i.hp||0),0);
 const gearCrit=gear.reduce((s,i)=>s+(i.crit||0),0),gearSpeed=gear.reduce((s,i)=>s+(i.speed||0),0);
 const attack=Math.round((p.attack+gearAttack)*(1+(skills.strength||0)*.05));
 const maxHp=Math.round((p.maxHp+gearHp)*(1+(skills.survival||0)*.04));
 const defense=Math.round((p.level-1)*.8+gearDefense+(skills.survival||0)*1.25);
 const crit=clamp(5+p.level*.18+gearCrit+(skills.critical||0)*2,5,48);
 const dodge=clamp(2+p.level*.08+gearSpeed*.18,2,20);
 const attackInterval=clamp(CombatRules.playerAttackInterval*(1-Math.min(.32,(gearSpeed+p.level*.12)/100)),.43,.72);
 const lifeSteal=clamp((skills.absorb||0)*.012,0,.12);
 return{attack,maxHp,defense,crit,dodge,attackInterval,lifeSteal}
}
function rollPlayerDamage(){
 const s=playerCombatStats(),critical=Math.random()<s.crit/100;
 let damage=s.attack*(critical?1.72:1);
 state.rpg.hitSeq=(state.rpg.hitSeq||0)+1;
 if(state.player.level>=5&&state.rpg.hitSeq%5===0)damage*=1.35; // learned combat rhythm
 if(state.player.level>=15&&state.rpg.hitSeq%9===0)damage*=1.65; // heavy finisher
 return{damage:Math.max(1,Math.round(damage)),critical}
}
function clampPlayerHp(){
 const max=playerCombatStats().maxHp;state.player.hp=clamp(state.player.hp,0,max)
}
function rollRarity(monster){
 const rank=monster.userData.rank||'normal',lv=monster.userData.level||1,r=Math.random();
 let idx=0;
 if(rank==='prototype')idx=r<.18?4:r<.62?3:2;
 else if(rank==='elite')idx=r<.07?4:r<.32?3:r<.75?2:1;
 else if(rank==='mutant')idx=r<.08?3:r<.34?2:r<.76?1:0;
 else idx=r<.06?2:r<.24?1:0;
 if(lv>=15&&Math.random()<.10)idx=Math.min(4,idx+1);
 return RARITY_META[idx]
}
function gearInstance(template,rarity,level){
 const rm=rarityMeta(rarity),lvScale=1+Math.max(0,level-1)*.045,m=rm.mult*lvScale,id=`ITEM-${++state.rpg.itemSeq}`;
 return{id,templateId:template.id,name:template.name,slot:template.slot,rarity:rm.id,level,upgrade:0,
   attack:Math.round((template.attack||0)*m),defense:Math.round((template.defense||0)*m),hp:Math.round((template.hp||0)*m),
   crit:Number(((template.crit||0)*m).toFixed(1)),speed:Number(((template.speed||0)*m).toFixed(1)),
   price:Math.max(1,Math.round(template.price*rm.price*lvScale))}
}
function itemStatText(i){
 const a=[];if(i.attack)a.push(`공격 +${i.attack}`);if(i.defense)a.push(`방어 +${i.defense}`);if(i.hp)a.push(`HP +${i.hp}`);if(i.crit)a.push(`치명 +${i.crit}%`);if(i.speed)a.push(`속도 ${i.speed>0?'+':''}${i.speed}`);
 if(i.upgrade)a.push(`강화 +${i.upgrade}`);return a.join(' · ')||'특수 효과'
}
function chooseGearTemplate(monster){
 const tier=monster.userData.rank==='prototype'?4:monster.userData.rank==='elite'?3:monster.userData.rank==='mutant'?2:Math.min(2,Math.floor(monster.userData.level/6));
 const eligible=ITEM_CATALOG.filter(i=>i.tier<=tier+1);
 return pick(eligible.length?eligible:ITEM_CATALOG)
}
function giveGear(item){
 if(state.rpg.inventory.length>=60){
   const silver=Math.round(item.price*.35);state.rpg.silver+=silver;return{autoSold:silver}
 }
 state.rpg.inventory.push(item);state.rpg.totalLoot++;state.rpg.lastDrop=item;return{item}
}
function rewardMonsterLoot(m){
 const u=m.userData,rank=MONSTER_RANKS[u.rank]||MONSTER_RANKS.normal;
 const silver=Math.max(1,Math.round((5+u.level*2.1)*rank.gold*(.8+Math.random()*.45)));
 const bio=Math.max(1,Math.round(rank.gold*(.7+Math.random()*1.2)));
 state.rpg.silver+=silver;state.rpg.bioFragments+=bio;
 if(Math.random()<.22*rank.gold)state.rpg.dataShards++;
 let dropText=`+${silver} 은화 · 생체파편 +${bio}`;
 if(Math.random()<rank.drop){
   const rarity=rollRarity(m),item=gearInstance(chooseGearTemplate(m),rarity.id,u.level),got=giveGear(item);
   dropText+=got.autoSold?` · 가방 가득 자동판매 +${got.autoSold}`:` · [${rarity.name}] ${item.name}`
 }
 if(Math.random()<.14+(.06*rank.gold)){
   state.rpg.potions=(state.rpg.potions||0)+1;dropText+=' · 회복약 +1'
 }
 if(!state.rpg.discoveries.includes(u.type)){
   state.rpg.discoveries.push(u.type);
   const cfg=MONSTER_CATALOG.find(x=>x.id===u.type);
   if(cfg)addLog('warn',`${cfg.code} 실험기록 복원`,`${cfg.name}의 조직에서 실험 표식과 기록 조각이 발견됐다. ${cfg.experiment}`,'SYSTEM','이 생물은 자연 발생종이 아니다.')
 }
 state.rpg.experimentKills[u.type]=(state.rpg.experimentKills[u.type]||0)+1;
 showEvent(dropText);return dropText
}
function equipItem(id){
 const idx=state.rpg.inventory.findIndex(i=>i.id===id);if(idx<0)return;
 const item=state.rpg.inventory.splice(idx,1)[0],old=state.rpg.equipment[item.slot];
 if(old)state.rpg.inventory.push(old);state.rpg.equipment[item.slot]=item;clampPlayerHp();renderRpgPanel();uiDirty=true;save()
}
function sellItem(id){
 const idx=state.rpg.inventory.findIndex(i=>i.id===id);if(idx<0)return;
 const [item]=state.rpg.inventory.splice(idx,1),gain=Math.max(1,Math.round(item.price*.48));state.rpg.silver+=gain;showEvent(`${item.name} 판매 · +${gain} 은화`);renderRpgPanel();save()
}
function upgradeEquipped(slot){
 const i=state.rpg.equipment[slot];if(!i)return;
 const n=(i.upgrade||0)+1,cost=55+n*55+Math.round(i.level*3),bio=3+n*2;
 if(state.rpg.silver<cost||state.rpg.bioFragments<bio){showEvent(`강화 재료 부족 · ${cost} 은화 / 파편 ${bio}`);return}
 state.rpg.silver-=cost;state.rpg.bioFragments-=bio;i.upgrade=n;
 for(const k of ['attack','defense','hp'])if(i[k])i[k]=Math.max(1,Math.round(i[k]*1.09));
 for(const k of ['crit','speed'])if(i[k])i[k]=Number((i[k]*1.05).toFixed(1));
 clampPlayerHp();showEvent(`${i.name} +${n} 강화 성공`);renderRpgPanel();save()
}
function buyPotion(){
 const cost=25+Math.floor(state.player.level*1.2);
 if(state.rpg.silver<cost){showEvent(`은화 부족 · ${cost} 필요`);return}
 state.rpg.silver-=cost;state.rpg.potions=(state.rpg.potions||0)+1;showEvent(`회복약 구입 · -${cost} 은화`);renderRpgPanel();save()
}
function usePotion(){
 if((state.rpg.potions||0)<=0){showEvent('회복약이 없습니다');return}
 const s=playerCombatStats();if(state.player.hp>=s.maxHp){showEvent('이미 체력이 가득합니다');return}
 state.rpg.potions--;state.player.hp=Math.min(s.maxHp,state.player.hp+Math.round(s.maxHp*.48));showEvent('회복약 사용');renderRpgPanel();uiDirty=true
}
function learnRpgSkill(key){
 const s=state.rpg.skills;if((state.rpg.skillPoints||0)<=0||!(key in s)||s[key]>=10)return;
 s[key]++;state.rpg.skillPoints--;clampPlayerHp();renderRpgPanel();uiDirty=true;save()
}
function rarityClass(i){return`rarity-${i.rarity||'common'}`}
function renderRpgPanel(){
 if(!$('rpgPanel')||$('rpgPanel').classList.contains('hidden'))return;
 const s=playerCombatStats(),r=state.rpg;
 $('rpgSummary').innerHTML=[
  ['LV',state.player.level],['공격',s.attack],['방어',s.defense],['HP',`${Math.round(state.player.hp)}/${s.maxHp}`],['은화',r.silver],['생체파편',r.bioFragments]
 ].map(([a,b])=>`<span>${a}<b>${b}</b></span>`).join('');
 const slots=[['weapon','무기'],['armor','방어구'],['accessory','장신구']];
 $('rpgEquipment').innerHTML=slots.map(([k,n])=>{const i=r.equipment[k];return`<div class="rpg-equipment-row"><div><b>${n} · ${i?`<span class="${rarityClass(i)}">${i.name}</span>`:'비어 있음'}</b><small>${i?`LV.${i.level} · ${itemStatText(i)}`:'몬스터 사냥으로 장비 획득'}</small></div>${i?`<button data-upgrade="${k}">강화</button>`:''}</div>`}).join('');
 const skillMeta={strength:['근력 단련','랭크당 최종 공격력 +5%'],survival:['생존 본능','랭크당 최대 HP +4% · 방어 증가'],critical:['치명 감각','랭크당 치명타 확률 +2%'],absorb:['생체 흡수','랭크당 피해의 1.2% 체력 회복']};
 $('rpgSkills').innerHTML=`<small>사용 가능 포인트 ${r.skillPoints||0}</small>`+Object.entries(skillMeta).map(([k,v])=>`<div class="rpg-skill"><div><b>${v[0]} ${r.skills[k]}/10</b><small>${v[1]}</small></div><button data-skill="${k}" ${(r.skillPoints||0)<=0||r.skills[k]>=10?'disabled':''}>+1</button></div>`).join('');
 $('rpgInventory').innerHTML=r.inventory.length?r.inventory.map(i=>`<div class="rpg-item"><div><b class="${rarityClass(i)}">[${rarityMeta(i.rarity).name}] ${i.name}</b><small>LV.${i.level} · ${itemStatText(i)}</small></div><div class="rpg-item-actions"><button data-equip="${i.id}">장착</button><button data-sell="${i.id}">판매</button></div></div>`).join(''):`<div class="empty-rpg">아직 획득한 장비가 없습니다.</div>`;
 const pcost=25+Math.floor(state.player.level*1.2);
 $('rpgShop').innerHTML=`<div class="rpg-shop-row"><div><b>응급 회복약 ×1</b><small>최대 HP의 48% 회복 · 보유 ${r.potions||0}</small></div><button id="buyPotionBtn">${pcost} 은화</button></div><div class="rpg-shop-row"><div><b>회복약 즉시 사용</b><small>전투 중에도 사용 가능</small></div><button id="usePotionBtn">사용</button></div>`;
 $('rpgLab').innerHTML=MONSTER_CATALOG.map(c=>{const seen=r.discoveries.includes(c.id),kills=r.experimentKills[c.id]||0;return`<div class="lab-entry"><div><b>${seen?`${c.code} · ${c.name}`:'??? · 미분석 실험체'}</b><small>${seen?`${c.experiment} · 처치 ${kills}회`:'처치 후 조직과 기록을 분석해야 정보가 열린다.'}</small></div></div>`}).join('');
 $('rpgPanel').querySelectorAll('[data-equip]').forEach(b=>b.onclick=()=>equipItem(b.dataset.equip));
 $('rpgPanel').querySelectorAll('[data-sell]').forEach(b=>b.onclick=()=>sellItem(b.dataset.sell));
 $('rpgPanel').querySelectorAll('[data-upgrade]').forEach(b=>b.onclick=()=>upgradeEquipped(b.dataset.upgrade));
 $('rpgPanel').querySelectorAll('[data-skill]').forEach(b=>b.onclick=()=>learnRpgSkill(b.dataset.skill));
 $('buyPotionBtn').onclick=buyPotion;$('usePotionBtn').onclick=usePotion
}
...
function pickMonsterConfig(){return {...pick(MONSTER_CATALOG)}}
function randomMonsterSpawn(){
 const a=Math.random()*Math.PI*2,r=rand(52,185);
 return new THREE.Vector3(Math.cos(a)*r,0,Math.sin(a)*r*.82)
}
function experimentMonsterLevel(spawn){
 const d=Math.hypot(spawn.x,spawn.z),distanceTier=Math.max(0,Math.floor((d-45)/36))*2;
 const eraTier=Math.min(18,Math.floor(Math.max(0,state.year-55)/8));
 return clamp(1+distanceTier+eraTier,1,70)
}
function rollMonsterRank(lvl){
 const r=Math.random();
 if(lvl>=9&&r<.025)return'prototype';
 if(r<.10)return'elite';
 if(r<.30)return'mutant';
 return'normal'
}
function monsterMaterial(color,glow=0){
 return new THREE.MeshStandardMaterial({color,roughness:.78,metalness:.05,emissive:glow||0,emissiveIntensity:glow?.26:0})
}
function addMonsterEyes(g,y,z,glow){
 const eye=new THREE.MeshBasicMaterial({color:glow||0xff5666});
 for(const x of[-.13,.13]){const e=new THREE.Mesh(new THREE.SphereGeometry(.045,6,4),eye);e.position.set(x,y,z);g.add(e)}
}
function buildExperimentMonsterVisual(cfg,rank,mutation){
 const g=new THREE.Group(),body=monsterMaterial(cfg.bodyColor||0x454545),dark=monsterMaterial(0x202329),glow=cfg.glowColor||0xc85f72;
 const bone=monsterMaterial(0xc2b496),energy=new THREE.MeshBasicMaterial({color:glow,transparent:true,opacity:.78});
 const shape=cfg.shape||'brute';
 if(shape==='brute'){
   const torso=new THREE.Mesh(new THREE.SphereGeometry(.72,10,8),body);torso.scale.set(1.35,1.35,.86);torso.position.y=1.35;g.add(torso);
   const head=new THREE.Mesh(new THREE.SphereGeometry(.38,9,7),dark);head.position.set(0,2.18,.18);g.add(head);addMonsterEyes(g,2.22,.53,glow);
   for(const x of[-.72,.72]){const arm=new THREE.Mesh(new THREE.CylinderGeometry(.18,.28,1.6,7),body);arm.position.set(x,1.25,.05);arm.rotation.z=x<0?-.22:.22;g.add(arm)}
   for(const x of[-.3,.3]){const leg=new THREE.Mesh(new THREE.CylinderGeometry(.18,.23,1.15,7),dark);leg.position.set(x,.55,0);g.add(leg)}
 }else if(shape==='crawler'){
   const torso=new THREE.Mesh(new THREE.SphereGeometry(.58,10,7),body);torso.scale.set(1.55,.62,1.9);torso.position.y=.65;g.add(torso);
   const head=new THREE.Mesh(new THREE.SphereGeometry(.34,8,6),dark);head.position.set(0,.78,.95);g.add(head);addMonsterEyes(g,.85,1.25,glow);
   for(const[x,z]of[[-.5,.55],[.5,.55],[-.5,-.55],[.5,-.55]]){const leg=new THREE.Mesh(new THREE.CylinderGeometry(.09,.13,.9,6),dark);leg.position.set(x,.35,z);leg.rotation.z=x<0?.55:-.55;g.add(leg)}
   for(const x of[-.48,.48]){const blade=new THREE.Mesh(new THREE.ConeGeometry(.09,1.0,5),bone);blade.position.set(x,.55,1.15);blade.rotation.x=Math.PI/2;g.add(blade)}
 }else if(shape==='spore'){
   const sac=new THREE.Mesh(new THREE.SphereGeometry(.85,12,9),body);sac.scale.set(1.05,1.3,1.05);sac.position.y=1.1;g.add(sac);
   for(let i=0;i<7;i++){const a=i/7*Math.PI*2,stalk=new THREE.Mesh(new THREE.CylinderGeometry(.035,.075,.72,5),dark);stalk.position.set(Math.cos(a)*.52,1.85,Math.sin(a)*.52);stalk.rotation.z=Math.cos(a)*.35;g.add(stalk);const cap=new THREE.Mesh(new THREE.SphereGeometry(.14,6,5),energy);cap.position.set(Math.cos(a)*.7,2.15,Math.sin(a)*.7);g.add(cap)}
   addMonsterEyes(g,1.25,.82,glow);
 }else if(shape==='conductor'){
   const torso=new THREE.Mesh(new THREE.CylinderGeometry(.25,.38,1.5,7),body);torso.position.y=1.25;g.add(torso);
   const head=new THREE.Mesh(new THREE.SphereGeometry(.28,8,6),dark);head.position.y=2.18;g.add(head);addMonsterEyes(g,2.22,.27,glow);
   for(const x of[-.48,.48]){const arm=new THREE.Mesh(new THREE.CylinderGeometry(.06,.09,1.35,6),dark);arm.position.set(x,1.35,0);arm.rotation.z=x<0?-.3:.3;g.add(arm);const orb=new THREE.Mesh(new THREE.SphereGeometry(.11,8,6),energy);orb.position.set(x*1.35,.82,.05);g.add(orb)}
   for(const x of[-.18,.18]){const leg=new THREE.Mesh(new THREE.CylinderGeometry(.08,.1,1.25,6),dark);leg.position.set(x,.55,0);g.add(leg)}
   const core=new THREE.Mesh(new THREE.SphereGeometry(.13,8,6),energy);core.position.set(0,1.45,.32);g.add(core)
 }else if(shape==='horned'){
   const torso=new THREE.Mesh(new THREE.SphereGeometry(.76,11,8),body);torso.scale.set(1.45,.85,1.55);torso.position.y=1.0;g.add(torso);
   const head=new THREE.Mesh(new THREE.SphereGeometry(.45,9,7),body);head.position.set(0,1.25,.9);g.add(head);addMonsterEyes(g,1.35,1.31,glow);
   for(const[x,z]of[[-.5,.45],[.5,.45],[-.5,-.45],[.5,-.45]]){const leg=new THREE.Mesh(new THREE.CylinderGeometry(.13,.18,.85,6),dark);leg.position.set(x,.42,z);g.add(leg)}
   for(const x of[-.24,.24]){const horn=new THREE.Mesh(new THREE.ConeGeometry(.13,.85,6),bone);horn.position.set(x,1.68,1.0);horn.rotation.x=-.62;g.add(horn)}
   for(let i=0;i<5;i++){const spike=new THREE.Mesh(new THREE.ConeGeometry(.09,.48,5),bone);spike.position.set(0,1.65,-.55+i*.28);spike.rotation.x=-.15;g.add(spike)}
 }else{
   const torso=new THREE.Mesh(new THREE.BoxGeometry(.72,1.25,.4),body);torso.position.y=1.35;g.add(torso);
   const head=new THREE.Mesh(new THREE.SphereGeometry(.32,9,7),dark);head.position.y=2.2;g.add(head);addMonsterEyes(g,2.22,.31,glow);
   for(const x of[-.48,.48]){const arm=new THREE.Mesh(new THREE.CylinderGeometry(.07,.1,1.25,6),dark);arm.position.set(x,1.48,0);arm.rotation.z=x<0?-.18:.18;g.add(arm);const blade=new THREE.Mesh(new THREE.ConeGeometry(.08,.8,5),energy);blade.position.set(x*1.12,.72,.05);blade.rotation.z=x<0?.25:-.25;g.add(blade)}
   for(const x of[-.2,.2]){const leg=new THREE.Mesh(new THREE.CylinderGeometry(.1,.12,1.2,6),dark);leg.position.set(x,.56,0);g.add(leg)}
 }
 if(rank!=='normal'){
   const aura=new THREE.Mesh(new THREE.RingGeometry(.75,1.0,24),new THREE.MeshBasicMaterial({color:glow,transparent:true,opacity:rank==='prototype'?.7:.38,side:THREE.DoubleSide}));
   aura.rotation.x=-Math.PI/2;aura.position.y=.04;g.add(aura)
 }
 if(mutation){
   const marker=new THREE.Mesh(new THREE.TorusGeometry(.28,.035,6,16),new THREE.MeshBasicMaterial({color:glow,transparent:true,opacity:.72}));
   marker.position.set(0,2.55,0);marker.rotation.x=Math.PI/2;g.add(marker)
 }
 return g
}
...
function createMonster(forceId=null){
 const cfg=(forceId&&MONSTER_CATALOG.find(c=>c.id===forceId))||pickMonsterConfig(),spawn=randomMonsterSpawn(),lvl=experimentMonsterLevel(spawn),rank=rollMonsterRank(lvl),rankM=MONSTER_RANKS[rank];
 ...
 g.userData={
   ...
   expBase:cfg.expBase||20,expPerLevel:cfg.expPerLevel||8,respawnMs:cfg.respawnMs||7000,
...
function monsterEraTargetCount(){
 if(state.year<55)return 0;if(state.year<57)return 1;if(state.year<60)return 3;if(state.year<65)return 5;return 7
}
function allowedMonsterFamiliesForYear(){
 if(state.year<55)return[];
 if(state.year<57)return['orc_01'];
 if(state.year<60)return['orc_01','failed_01'];
 if(state.year<65)return['orc_01','failed_01','muscle_17','cutter_04'];
 return MONSTER_CATALOG.map(c=>c.id)
}
function ensureMonsterEra(){
 if(state.year<55)return;
 if(!state.player.awakened){state.player.awakened=true;state.player.hp=playerCombatStats().maxHp;state.bio??=defaultBioState();state.bio.firstOrc=true}
 if(!monsters.length)createMonster('orc_01');
 const target=monsterEraTargetCount(),allowed=allowedMonsterFamiliesForYear();
 while(monsters.length<target){const id=pick(allowed);createMonster(id)}
}
function gainPlayerExp(n){
 ...
 while(state.player.exp>=state.player.nextExp){
   ...
   state.player.maxHp+=14;state.player.attack+=4;state.rpg.skillPoints=(state.rpg.skillPoints||0)+1;
   state.player.hp=playerCombatStats().maxHp;
   showEvent(`LEVEL UP · LV.${state.player.level} · 공격/HP 상승 · 스킬포인트 +1`);
 }
}
...
function respawnPlayer(){
 state.player.dead=false;state.player.hp=playerCombatStats().maxHp;state.player.respawnAt=0;
...
function setMonsterTarget(m){
 if((state.observerTransport?.mode||'walk')!=='walk')setObserverTransport('walk');
...
function updatePlayer(dt,now){
 ...
 if(manualMove.active){
   ...
   if(mag>.04){
     ...
     player.position.addScaledVector(d,CombatRules.playerMoveSpeed*(.55+.45*mag)*scale*observerTransportMultiplier()*dt);
 ...
 if(observerTransport){observerTransport.position.set(player.position.x,0,player.position.z);observerTransport.rotation.y=player.rotation.y}
 observerWorldLocation();
     followId='PLAYER';if($('followSelect'))$('followSelect').value='PLAYER';if(camMode!=='follow')setCameraMode('follow');
     return
   }
 }
 ...
 if((wantsCombat||ud.moving)&&dist>stop){
   ...
   player.position.addScaledVector(d,CombatRules.playerMoveSpeed*scale*observerTransportMultiplier()*dt);player.rotation.y=Math.atan2(d.x,d.z);
   ...
   if(wantsCombat){
     ...
     if(ud.attackCooldown<=0){
       const stats=playerCombatStats(),hit=rollPlayerDamage();ud.attackCooldown=stats.attackInterval;ud.attackAnim=.32;
       selectedMonster.userData.hp-=hit.damage;
       if(stats.lifeSteal>0)state.player.hp=Math.min(stats.maxHp,state.player.hp+Math.max(1,Math.round(hit.damage*stats.lifeSteal)));
       if(hit.critical)showEvent(`치명타 ${hit.damage}`);
...
function updateMonsters(dt,now){
 ...
 for(const m of monsters){
   ...
   if(ud.mutation?.id==='regen'&&ud.hp<ud.maxHp&&!ud.fsm.is(MonsterStates.ATTACK))ud.hp=Math.min(ud.maxHp,ud.hp+dt*ud.maxHp*.008);
   ...
   if(ud.fsm.is(MonsterStates.CHASE)){
     ...
     if(ud.attackCooldown<=0&&dToPlayer<=ud.attackRange*1.12){
       ...
       if(Math.random()>=ps.dodge/100){
         const raw=CombatRules.monsterDamage(ud.attackPower,ud.level),taken=Math.max(1,Math.round(raw-ps.defense*.62));
         state.player.hp=Math.max(0,state.player.hp-taken);
         if(ud.mutation?.id==='leech')ud.hp=Math.min(ud.maxHp,ud.hp+Math.round(taken*.22));
...
function humanModel(r){
 ...
 g.position.copy(g.userData.home);const age=Number(r.age||0),bodyScale=bodyScaleForAge(age)*HUMAN_WORLD_SCALE;g.scale.setScalar(bodyScale);scene.add(g);return g
}
const people=state.residents.filter(r=>(r.homeCity||'L001')==='L001').map(humanModel);const personMap=new Map(people.map(p=>[p.userData.id,p]));

let myeongjaGrave=null;
function ensureMyeongjaGraveVisual(){return}
...
function processMyeongjaLifecycle(){return false}
ensureMyeongjaGraveVisual();


function refreshFollowSelect(){const current=followId||'PLAYER';$('followSelect').innerHTML=`<option value="PLAYER">⚔ 나 · 관찰자</option><option value="BOKSHIL">🐕 복실이 · 마을 순찰</option>`+state.residents.filter(r=>(r.homeCity||'L001')==='L001').map(r=>`<option value="${r.id}">${r.name} · ${r.job}</option>`).join('');if(current==='PLAYER'||current==='BOKSHIL'||personMap.has(current))$('followSelect').value=current}
function spawnResidentVisual(r){if((r.homeCity||'L001')!=='L001')return null;if(personMap.has(r.id))return personMap.get(r.id);const p=humanModel(r);people.push(p);personMap.set(r.id,p);return p}
...
function updateLifeStages(){
 for(const r of state.residents){
  ...
  const p=personMap.get(r.id);if(p){const sc=bodyScaleForAge((r.age||0)+(state.day-1)/365)*HUMAN_WORLD_SCALE;p.scale.setScalar(sc);p.userData.job=r.job}
 }
}
...
function createLocalDescendant(mother,father){
 ...
 r.parents=[mother.id,father.id];r.partnerId=null;r.homeCity=mother.homeCity||father.homeCity||'L001';r.birthYear=state.year;r.childrenTarget=3+(stableHash01(id+'family')>.48?1:0);state.residents.push(r);spawnResidentVisual(r);state.demography.births++;return r
}
...
function syncOfficialPopulationRuntime(announce=true){
 state.demography.children=state.residents.filter(r=>r.age<16).length;return 0
}
let selectedBrainId=null;

function actionTarget(action,r,partner=null){
 const ownHome=personMap.get(r.id)?.userData.home||residentHomePosition(r);
 if(action==='eat'||action==='rest')return spreadAround(r,ownHome,.3,2.8,action);
 if(action==='cook')return spreadAround(r,state.buildings.kitchen?LOC.center:ownHome,4,14,'cook');
 if(action==='socialize'){
   if(partner){const pp=personMap.get(partner.id);if(pp)return spreadAround(r,pp.position,1.2,4.5,'social')}
   return spreadAround(r,LOC.meeting,7,22,'social')
 }
 if(action==='forage')return spreadAround(r,LOC.herbs,8,25,'forage');
 if(action==='water'||action==='waterwork')return distributedRiverTarget(r,action);
 if(action==='wood')return spreadAround(r,LOC.forest,12,32,'wood');
 if(action==='stone')return spreadAround(r,LOC.stone,10,28,'stone');
 if(action==='farm'){const fi=Math.floor(residentStable01(r,'farm-i')*Math.max(1,state.buildings.field||1)),s=fieldSpot(fi);return spreadAround(r,new THREE.Vector3(s[0],0,s[1]),1.5,7,'farm')}
 if(action==='animals')return spreadAround(r,bokshil?.position||LOC.center,2.5,8,'animals');
 if(action==='pottery'||action==='record')return spreadAround(r,state.buildings.workshop?LOC.workshop:LOC.meeting,3,12,action);
 if(action==='build'){const need=Math.max(2,Math.ceil(state.residents.length/4.6)),s=houseSpot(Math.min(state.buildings.house||0,need-1));return spreadAround(r,new THREE.Vector3(s[0],0,s[1]),1,6,'build')}
 if(action==='help'&&partner){const pp=personMap.get(partner.id);return spreadAround(r,pp?.position||LOC.center,1,4,'help')}
 if(action==='learn')return spreadAround(r,LOC.meeting,8,24,'learn');
 if(action==='explore'){const aa=residentStable01(r,`explore-${state.year}`)*Math.PI*2,rr=45+residentStable01(r,'explore-r')*95;return new THREE.Vector3(Math.cos(aa)*rr,0,Math.sin(aa)*rr*.78)}
 return spreadAround(r,ownHome,1,6,'idle')
}
...
function desiredActionByJob(r){
 ...
 if(j.includes('동물')||j.includes('생명'))return 'animals';
...
function applyActionEffect(p){
 ...
 else if(a==='animals'){state.bio??=defaultBioState();state.bio.score+=.12*eff;state.animalStats.care=clamp((state.animalStats.care||0)+.4*eff,0,100);if(state.bio.livestock){ensureDomesticAnimals();gain({labor:.12*eff,food:state.buildings.pen?.06*eff:0})}else gain({labor:.08*eff});n.curiosity-=20;n.safety-=5;if(Math.random()<.025)meaningfulEvent(r,'복실이의 생명구조를 기록하다',`${r.name}이 복실이의 털과 행동, 자연스럽게 떨어진 세포를 기록했다. 해부나 강제 실험은 허용되지 않았다.`)}
...
function maybeEmergeRole(r){
 ...
 const options=[
  ...
  [state.bio?.artificialDog?'인공생명 돌봄이':'생명관찰자','animals','사육'],['도공','pottery','도공'],['기록자','record','기록'],['목수','build','목공'],['길잡이','explore','채집']
 ];
 let best=null,bs=0;
...
function handleTap(e){
 ...
 if(bhit){followId='BOKSHIL';if($('followSelect'))$('followSelect').value='BOKSHIL';setCameraMode('follow');showEvent('복실이 · 이 세계에서 인간이 만들지 않은 유일한 비인간 동물');return}
...
function closeRpgPanel(e){
 ...
 $('rpgPanel').classList.add('hidden');
}
$('rpgClose').onclick=closeRpgPanel;
$('rpgClose').addEventListener('pointerup',closeRpgPanel);

$('rpgPanel').addEventListener('click',e=>{if(e.target===$('rpgPanel'))closeRpgPanel(e)});
$('rpgPanel').addEventListener('pointerdown',e=>e.stopPropagation());
...
const TECH_RULES={
 ...
 사육기초:{skill:'사육',base:.13,req:()=>!!state.bio?.artificialDog,desc:'최초 인공동물 이후 먹이·휴식·안전거리를 체계화한다.'},
...
function dailySettlementEconomy(){
 ...
 const farmWeather=weatherFarmFactor();
 gain({food:.035*workers.length+fields*(.42+ag*.0045)*farmWeather+(state.buildings.kitchen?.12:0)+(state.tech.공동취사.open?.08:0),
...
const WORLD_REGION_JOBS={
 ...
 '카르딘 산맥권':['광부','석공','제련공','목수','경비','운반장치기술자'],
 '세라칸 대초원':['풍력기술자','궤도운반기술자','토양관리자','천막장인','채집가','경비'],
 '네레이아 해권':['선박목수','염전일꾼','항해견습','해안농업인','의료기록자','저장기술자'],
 '솔라크 사막권':['화학기록자','우물관리자','직조인','저장기술자','경비','채집가'],
 '드라바스 화산군도':['제련공','광부','화산감시자','고온재료기술자','선박목수','농부'],
 '루메라 부유제도':['바람관측자','수학기록자','천문관측자','풍력기술자','목수','농부']
};
...
function activeCountryNames(){return Object.entries(state.world.countries||{}).filter(([n,c])=>c&&c.active!==false).map(([n])=>n)}
function cityController(cityOrId){const id=typeof cityOrId==='string'?cityOrId:cityOrId?.id;const city=typeof cityOrId==='string'?WORLD_CITY_BY_ID.get(cityOrId):cityOrId;return state.world.cityControl?.[id]||city?.country||null}
function controlledCities(country){return WORLD_DATA.cities.filter(c=>cityController(c)===country)}
function foundedControlledCities(country){return controlledCities(country).filter(c=>state.world.foundedCities.includes(c.id))}
function worldPersonById(id){return id?state.worldPeople.find(p=>p.id===id):null}
function worldChildrenOf(id){return state.worldPeople.filter(p=>p.alive!==0&&(p.parents||[]).includes(id))}
function personAncestorIds(person,depth=2,out=new Set()){if(!person||depth<=0)return out;for(const id of person.parents||[]){if(!id||out.has(id))continue;out.add(id);personAncestorIds(worldPersonById(id),depth-1,out)}return out}
function worldCloseKin(a,b){if(!a||!b||a.id===b.id)return true;const aa=personAncestorIds(a,2,new Set()),bb=personAncestorIds(b,2,new Set());if(aa.has(b.id)||bb.has(a.id))return true;for(const x of aa)if(bb.has(x))return true;return false}
function migrateWorldGenealogy(){
 if(state.world.genealogyMigrated)return;
 const groups=new Map();for(const p of state.worldPeople){p.parents??=[];p.partnerId??=null;p.lineage??=p.f||p.id;const k=`${p.c}|${p.f||p.id}`;if(!groups.has(k))groups.set(k,[]);groups.get(k).push(p)}
 for(const ps of groups.values()){
   const adults=ps.filter(p=>p.a>=18),m=adults.find(p=>p.g==='M'),f=adults.find(p=>p.g==='F');
   if(m&&f&&m.id!==f.id){m.partnerId=f.id;f.partnerId=m.id;for(const ch of ps.filter(p=>p.a<18)){if(!ch.parents.length)ch.parents=[f.id,m.id]}}
 }
 state.world.genealogyMigrated=true
}
function pairWorldHouseholds(country){
 const alive=aliveWorldPeople(country);for(const p of alive){if(p.partnerId&&!worldPersonById(p.partnerId)?.alive)p.partnerId=null}
 const women=alive.filter(p=>p.g==='F'&&p.a>=18&&p.a<=46&&!p.partnerId).sort((a,b)=>a.id.localeCompare(b.id));
 const men=alive.filter(p=>p.g==='M'&&p.a>=18&&p.a<=52&&!p.partnerId).sort((a,b)=>a.id.localeCompare(b.id));
 let made=0;for(const f of women){let best=null,score=999;for(const m of men){if(m.partnerId||worldCloseKin(f,m))continue;const s=Math.abs(f.a-m.a)+(f.ct===m.ct?0:7)+stableHash01(f.id+m.id);if(s<score){score=s;best=m}}if(!best)continue;f.partnerId=best.id;best.partnerId=f.id;const fam=`WF-${[f.id,best.id].sort().join('-')}`;f.f=best.f=fam;f.lineage=best.lineage=fam;made++;if(made>=Math.max(1,Math.ceil(alive.length/25)))break}
}
...
function chooseCountryLeader(name,reason='정기 선출'){
 const c=state.world.countries[name];if(!c||c.active===false)return null;let candidates=aliveWorldPeople(name).filter(p=>p.a>=22&&p.a<=74);
 if(c.politics?.form==='왕정'){
   const old=worldPersonById(c.politics.leaderId),kids=old?worldChildrenOf(old.id).filter(p=>p.alive!==0&&p.a>=18):[];if(kids.length)candidates=kids
 }
 candidates.sort((a,b)=>leaderScore(b,c)-leaderScore(a,c));const p=candidates[0]||aliveWorldPeople(name)[0]||null;c.politics??={form:'공동회의',leaderId:null,leaderSince:state.year,legitimacy:60,unrest:20,factions:{},history:[]};
 c.politics.leaderId=p?.id||null;c.politics.leaderSince=state.year;c.politics.legitimacy=clamp((c.politics.legitimacy||55)+6,0,100);c.politics.history??=[];c.politics.history.unshift({y:state.year,type:'leader',id:p?.id||null,name:p?.n||'공석',reason});c.politics.history=c.politics.history.slice(0,20);
 if(p)recordCountryEvent(name,`${reason}: ${p.n}(${p.a}세) 지도자`);return p
}
function ensurePoliticsInitialized(){
 for(const name of activeCountryNames()){const c=state.world.countries[name];c.politics??={form:'공동회의',leaderId:null,leaderSince:state.year,legitimacy:66,unrest:14,factions:{생활민:55,장인:45,교역민:35,무장대:28},history:[]};c.politics.factions??={생활민:55,장인:45,교역민:35,무장대:28};if(!countryLeaderPerson(name))chooseCountryLeader(name,'초기 대표 선정')}
}
...
function cloneSuccessorCountry(parentName,newName,region,capitalId){
 const p=state.world.countries[parentName],scale=.38;state.world.countries[newName]={...JSON.parse(JSON.stringify(p)),active:true,bornYear:state.year,dissolvedYear:null,origin:parentName,region,capitalId,population:0,lastPopulation:0,births:0,deaths:0,migration:0,settlements:1,knownCountries:[newName,parentName,...(p.knownCountries||[]).filter(n=>n!==newName)],events:[],history:[],causal:[],tradeRoutes:[],treaties:[],diplomacy:{},refugeesIn:0,refugeesOut:0,army:{mobilized:0,supply:100,morale:65,casualties:0,veterans:0},politics:{form:'공동회의',leaderId:null,leaderSince:state.year,legitimacy:54,unrest:28,factions:{생활민:50,장인:45,교역민:35,무장대:35},history:[]}};
...
function simulatePoliticsAnnual(){
 ensurePoliticsInitialized();for(const name of activeCountryNames()){
   const c=state.world.countries[name],pol=c.politics;const desired=desiredGovernment(name,c);if(desired!==pol.form&&stableHash01(`${name}|${state.year}|gov`)>.56){const old=pol.form;pol.form=desired;recordCountryEvent(name,`정체 변화 ${old} → ${desired}`);pol.history.unshift({y:state.year,type:'form',from:old,to:desired})}
   const leader=countryLeaderPerson(name);if(!leader||leader.a>76)chooseCountryLeader(name,leader?'지도자 교체':'지도자 공석 승계');
...
function syncLocalWorldPeople(){
 ...
 for(const p of state.worldPeople){if(!p.local)continue;if(!liveIds.has(p.id)){p.alive=0;p.dy??=state.year}}
 for(const r of state.residents){
   r.homeCity??='L001';let p=state.worldPeople.find(x=>x.id===r.id);
   if(!p){p={id:r.id,n:r.name,c:'에르단 왕국',r:'아르케아 중앙대륙',ct:r.homeCity,a:r.age,g:r.gender==='여성'?'F':'M',j:r.job,f:r.family||r.id,lineage:r.family||r.id,parents:r.parents||[],partnerId:r.partnerId||null,by:state.year-r.age,local:1,alive:1};state.worldPeople.push(p)}
   else{p.n=r.name;p.a=r.age;p.j=r.job;p.f=r.family||p.f;p.lineage=r.family||p.lineage;p.parents=r.parents||p.parents||[];p.partnerId=r.partnerId||null;p.alive=1;p.local=1;p.c='에르단 왕국';p.r='아르케아 중앙대륙';p.ct=r.homeCity}
 }
}
...
function simulateAggregateDemography(name,c){
 ...
 const birthRate=clamp(.044+(pros-50)*.000065+(stab-50)*.000055-(war?.006:0),.030,.056);
 const deathRate=clamp(.014+(50-food)*.000085+(50-stab)*.000050+(war?.0035:0),.008,.030);
...
function createWorldBaby(country,mother,father=null){
 ...
 return{id:`WB${seq}`,n:worldPersonName(region,seq),c:country,r:region,ct:mother?.ct||father?.ct||countryNearestCity(country)?.id||'L001',
   a:0,g:Math.random()<.5?'F':'M',j:'유아',f:fam,lineage:fam,parents:[mother?.id,father?.id].filter(Boolean),partnerId:null,birthCountry:country,by:state.year,alive:1}
}
...
function updateCountryEconomy(name,c){
 ...
 e.grain=clamp(e.grain+labor*9.2*soil-3.4-(war?2.1:0),0,180);e.wood=clamp(e.wood+labor*5.6*forestFactor-1.7,0,160);e.stone=clamp(e.stone+labor*3.5-1.1,0,140);e.metal=clamp(e.metal+labor*(/산맥|화산/.test(region)?3.5:1.0)-.8,0,120);
 if(state.bio?.livestock){e.livestock=clamp(e.livestock+labor*(/대초원|아르케아/.test(region)?3.0:1.2)-.9,0,130);e.fish=clamp(e.fish+labor*(/해권|군도/.test(region)?2.5:.4)-.6,0,110)}else{e.livestock=0;e.fish=0}
 e.cloth=clamp(e.cloth+labor*(state.bio?.livestock?1.7:.7)-.6,0,110);c.food=clamp(e.grain*.72+(e.livestock+e.fish)*.32,0,100)
}
...
function maybeTradeAndTreaties(){
 const names=activeCountryNames();
...
function simulateCountryEcology(name,c){
 const e=c.ecology;if(!e)return;const climate=/사막/.test(c.region)?.65:/수림/.test(c.region)?1.18:/화산/.test(c.region)?.72:1;
 const humanPressure=Math.max(.08,Math.log10((c.population||1)+10)*.16+(c.infrastructure||0)/260);
 e.rabbit=e.deer=e.boar=e.wolf=0;e.forest=clamp(e.forest+(climate-1)*1.1-humanPressure*.48,8,100);e.soil=clamp(e.soil+(e.forest-50)*.01-humanPressure*.15,8,100);
 e.water=clamp(e.water+(/해권|수림/.test(c.region)?.35:/사막/.test(c.region)?-.45:0)-humanPressure*.14,5,100);e.pressure=clamp(humanPressure*14+(100-e.forest)*.10,0,100)
}

function annualCountryPopulationSimulation(){
 normalizeWorldState();syncLocalWorldPeople();
 for(const name of activeCountryNames()){
   const c=state.world.countries[name];c.births=0;c.deaths=0;c.migration=0;pairWorldHouseholds(name);const war=countryAtWar(name);
   if(name==='에르단 왕국'){
     const prev=Math.max(1,c.population||30),d=c.demography??=demographicSnapshot(prev,name),detail=state.residents.length;
     if(detail<LOCAL_DETAIL_CAP){c.population=detail;d.population=detail;c.births=Math.max(0,state.demography.births||0);c.deaths=Math.max(0,prev+c.births-detail)}
     else simulateAggregateDemography(name,c);
   }else{
     const tracked=aliveWorldPeople(name);
     ...
     pairWorldHouseholds(name);const alive=aliveWorldPeople(name),mothers=alive.filter(p=>p.g==='F'&&p.a>=18&&p.a<=42&&p.partnerId&&worldPersonById(p.partnerId)?.alive!==0);
     if(alive.length<80&&mothers.length){const sampleBirths=Math.min(2,Math.max(0,Math.round(alive.length*.025)));for(let i=0;i<sampleBirths;i++){const mother=mothers[i%mothers.length],father=worldPersonById(mother.partnerId);state.worldPeople.push(createWorldBaby(name,mother,father))}}
     simulateAggregateDemography(name,c)
   }
 ...
 for(const name of activeCountryNames()){const c=state.world.countries[name];c.history??=[];c.history.push([state.year,c.population,c.births,c.deaths,0,Math.round(c.food),Math.round(c.tech),Math.round(c.military),c.settlements||1]);if(c.history.length>100)c.history=c.history.slice(-100)}
 state.demography.births=0
...
function activeExpeditionBetween(a,b){return (state.world.expeditions||[]).find(e=>e.active&&((e.originCountry===a&&e.country===b)||(e.originCountry===b&&e.country===a)))}
function selectForeignExpeditionMembers(country,count=4){
 return aliveWorldPeople(country).filter(p=>p.a>=18&&p.a<=58&&!p.travelExpeditionId).sort((a,b)=>stableHash01(a.id+'exp')-stableHash01(b.id+'exp')).slice(0,count).map(p=>p.id)
}
function scheduleForeignCountryExpedition(fromCountry,toCountry){
 if(fromCountry===toCountry||activeExpeditionBetween(fromCountry,toCountry))return false;const from=countryCapital(fromCountry),to=countryCapital(toCountry);if(!from||!to)return false;
 const memberIds=selectForeignExpeditionMembers(fromCountry,4);if(memberIds.length<4)return false;const crossRegion=from.region!==to.region,kind=(/해권|군도|부유/.test(from.region)||/해권|군도|부유/.test(to.region)||crossRegion)?'sea':'land';
 const distanceKm=Math.round(cityDistanceKm(from.id,to.id)),travelDays=expeditionTravelDays(from.id,to.id,kind),startAbs=absDay(),exp={id:`EXP-F-${state.year}-${state.day}-${fromCountry}-${toCountry}`,targetType:'foreign-country',originCountry:fromCountry,country:toCountry,region:to.region,kind,from:from.id,to:to.id,city:to.id,startYear:state.year,startDay:state.day,startAbsDay:startAbs,arrivalAbsDay:startAbs+travelDays,distanceKm,speedKmDay:travelSpeedKmDay(kind),travelDays,active:true,completed:false,phase:'outbound',progress:0,memberIds};
 markExpeditionMembers(exp,true);state.world.expeditions.push(exp);recordCountryEvent(fromCountry,`${toCountry} 탐사 원정 출발 · ${distanceKm.toLocaleString()}km`);return true
}

function simulateCountryKnowledgeAnnual(){
 if(state.year<10)return;const names=activeCountryNames(),activeForeign=(state.world.expeditions||[]).filter(e=>e.active&&e.originCountry&&e.originCountry!=='에르단 왕국').length;
 if(activeForeign>=5)return;
 for(const name of names){
   ...
   if((state.world.expeditions||[]).some(e=>e.active&&e.originCountry===name))continue;
   const unknown=names.filter(n=>n!==name&&!c.knownCountries.includes(n)).sort((a,b)=>countryDistanceKm(name,a)-countryDistanceKm(name,b));if(!unknown.length)continue;
   const target=unknown[0],km=countryDistanceKm(name,target),reach=150+state.year*(24+c.tech*.22)+c.exploration*28+(c.infrastructure||0)*5;
   if(km<=reach)scheduleForeignCountryExpedition(name,target)
 }
}
function redistributeCityPopulation(name){
 const c=state.world.countries[name];if(!c||c.active===false)return;const cities=WORLD_DATA.cities.filter(x=>state.world.foundedCities.includes(x.id)&&cityController(x)===name);if(!cities.length)return;
 if(name==='에르단 왕국'){
   // Aggregate population follows the same city proportions as tracked families without double-counting them.
   const tracked=Object.fromEntries(cities.map(x=>[x.id,0]));for(const r of state.residents){r.homeCity??='L001';if(!(r.homeCity in tracked))r.homeCity=c.capitalId||cities[0].id;tracked[r.homeCity]++}
   const totalTracked=Math.max(1,Object.values(tracked).reduce((a,b)=>a+b,0)),out={};let used=0;
   cities.forEach((city,i)=>{const n=i===cities.length-1?Math.max(0,c.population-used):Math.max(0,Math.round(c.population*(tracked[city.id]||0)/totalTracked));out[city.id]=n;used+=n});
   if(!out[c.capitalId||'L001'])out[c.capitalId||'L001']=Math.max(1,out[c.capitalId||'L001']||0);c.cityPopulation=out;return
 }
 const cap=c.capitalId||cities[0].id,old=c.cityPopulation||{},weights=cities.map(city=>Math.max(1,old[city.id]||1)*(city.id===cap?1.15:1)),sum=weights.reduce((a,b)=>a+b,0),out={};let used=0;
 cities.forEach((city,i)=>{const v=i===cities.length-1?Math.max(0,c.population-used):Math.max(0,Math.round(c.population*weights[i]/sum));out[city.id]=v;used+=v});c.cityPopulation=out
}
function moveLocalHouseholdsToCity(fromId,toId,targetCount){
 const candidates=state.residents.filter(r=>(r.homeCity||'L001')===fromId&&r.id!=='C0001'),groups=new Map();
 for(const r of candidates){const k=r.family||r.id;if(!groups.has(k))groups.set(k,[]);groups.get(k).push(r)}
 const ordered=[...groups.values()].filter(g=>g.some(r=>(r.age||0)>=18)).sort((a,b)=>stableHash01((a[0]?.family||a[0]?.id)+toId)-stableHash01((b[0]?.family||b[0]?.id)+toId));let moved=[];
 for(const g of ordered){if(moved.length>=targetCount)break;if(moved.length+g.length>targetCount+3&&moved.length>=8)continue;for(const r of g){r.homeCity=toId;moved.push(r);const p=personMap.get(r.id);if(p){scene.remove(p);personMap.delete(r.id);const ix=people.indexOf(p);if(ix>=0)people.splice(ix,1)}}}
 syncLocalWorldPeople();refreshFollowSelect();return moved
}
function syncCountrySettlementGrowth(){
 for(const name of activeCountryNames()){
  const c=state.world.countries[name];if(!c)continue;let founded=WORLD_DATA.cities.filter(x=>state.world.foundedCities.includes(x.id)&&cityController(x)===name);redistributeCityPopulation(name);
  const target=clamp(Math.max(1,Math.ceil((c.population||1)/1000)),1,WORLD_DATA.cities.filter(x=>x.region===c.region).length);
  if(target>founded.length&&state.year-(c.lastSettlementSplitYear??-999)>=3){
   const slot=WORLD_DATA.cities.filter(x=>x.region===c.region&&!state.world.foundedCities.includes(x.id)).sort((a,b)=>a.id.localeCompare(b.id))[0];
   if(slot){const source=[...founded].sort((a,b)=>(c.cityPopulation?.[b.id]||0)-(c.cityPopulation?.[a.id]||0))[0]||countryCapital(name);state.world.foundedCities.push(slot.id);state.world.cityControl[slot.id]=name;c.lastSettlementSplitYear=state.year;
     if(name==='에르단 왕국'&&source){const trackedAtSource=state.residents.filter(r=>(r.homeCity||'L001')===source.id).length,moveN=clamp(Math.round(trackedAtSource*.18),8,18),moved=moveLocalHouseholdsToCity(source.id,slot.id,moveN);recordCountryEvent(name,`${source.name} 인구 성장 → ${slot.name} 가족 ${moved.length}명 분가`);addLog('story',`${slot.name} 분가 개척`,`${source.name}의 인구가 충분히 늘어난 뒤 ${moved.length}명이 가족 단위로 새 터에 옮겨갔다. 이주는 세계 인구를 늘리지 않았고, 기존 공동체에서 실제 사람이 빠져나갔다.`,'기록','사람이 먼저 늘고, 그 다음 마을이 갈라졌다.')}
     else recordCountryEvent(name,`${slot.name} 가족단위 분가 개척`);pushCountryCause(name,'인구가 먼저 증가함',`${slot.name} 가족단위 분가`,3)}
  }
  founded=WORLD_DATA.cities.filter(x=>state.world.foundedCities.includes(x.id)&&cityController(x)===name);c.settlements=founded.length;redistributeCityPopulation(name)
 }
}

function countriesInRegion(region){return activeCountryNames().filter(n=>state.world.countries[n]?.region===region)}
...
function simulateDiplomacyDaily(){
 ...
 for(const country of contactCountries()){
  ...
  if(state.year>=35&&readiness>=3&&d.relation<-72&&state.year-(d.lastWarEndYear??-999)>=12&&Math.random()<.000018*(1+meta.aggression/80))startLocalWar(country)
 }
}
...
function residentOnActiveExpedition(r){return !!(r?.travelExpeditionId&&(state.world.expeditions||[]).some(e=>e.id===r.travelExpeditionId&&e.active))}
function selectExpeditionMembers(kind='land',count=4){
 const available=state.residents.filter(r=>r.id!=='C0001'&&r.age>=18&&r.age<=58&&!residentOnActiveExpedition(r));
 available.sort((a,b)=>((b.brain?.action==='explore')-(a.brain?.action==='explore'))||((b.skills?.기록||0)+(b.skills?.채집||0)-(a.skills?.기록||0)-(a.skills?.채집||0)));
 return available.slice(0,Math.min(count,available.length)).map(r=>r.id)
}
function expeditionMemberNames(exp){return (exp.memberIds||[]).map(id=>state.residents.find(r=>r.id===id)?.name||worldPersonById(id)?.n).filter(Boolean)}
function markExpeditionMembers(exp,on=true){
 for(const id of exp.memberIds||[]){const r=state.residents.find(x=>x.id===id);if(r)r.travelExpeditionId=on?exp.id:null;const wp=worldPersonById(id);if(wp)wp.travelExpeditionId=on?exp.id:null;const p=personMap?.get?.(id);if(p)p.visible=!on}
}
function expeditionRemainingDays(exp){
 const now=absDay();
 if(exp.phase==='return')return Math.max(0,(exp.returnArrivalAbsDay||now)-now);
 return Math.max(0,(exp.arrivalAbsDay||now)-now)
}

function countryNearestCity(country){
 const start=WORLD_CITY_BY_ID.get('L001'),founded=foundedControlledCities(country);return founded.sort((a,b)=>worldPointDistanceKm(start,a)-worldPointDistanceKm(start,b))[0]||countryCapital(country)||null
}
...
function scheduleCountryExpedition(country,kind='land'){
 ...
 const exp={
  ...
  distanceKm,speedKmDay,travelDays,active:true,completed:false,phase:'outbound',progress:0,
  memberIds:selectExpeditionMembers(kind,kind==='sea'?5:4)
 };
 markExpeditionMembers(exp,true);
 state.world.expeditions.push(exp);
 addLog('story',`${country} 접촉 원정 출발`,`${state.year}년 ${state.day}일, ${expeditionMemberNames(exp).join('·')||'주민들'}이 ${country} 쪽으로 원정대를 꾸려 떠났다. 라엔 기준 약 ${distanceKm.toLocaleString()}km, 예상 이동 ${travelDays}일. 관찰자는 이미 그 나라를 볼 수 있지만 주민들은 실제 도착 전까지 외교 관계를 맺지 않는다.`,'관찰 AI','보이는 것과 갈 수 있는 것은 다른 문제였다.');
 return true
}
...
function scheduleWorldExpedition(region,kind='land'){
 ...
 const starter=INITIAL_REGION_COMMUNITIES.find(s=>s.region===region),c=starter?WORLD_CITY_BY_ID.get(starter.cityId):representativeCity(region);if(!c)return false;
 const from='L001',distanceKm=Math.round(cityDistanceKm(from,c.id)),speedKmDay=travelSpeedKmDay(kind),travelDays=expeditionTravelDays(from,c.id,kind),startAbs=absDay();
 const memberIds=selectExpeditionMembers(kind,kind==='sea'?5:4);if(memberIds.length<(kind==='sea'?5:4))return false;
 const exp={id:`EXP-${state.year}-${state.day}-${region}`,targetType:'region',region,kind,from,to:c.id,city:c.id,startYear:state.year,startDay:state.day,startAbsDay:startAbs,arrivalAbsDay:startAbs+travelDays,distanceKm,speedKmDay,travelDays,active:true,completed:false,phase:'outbound',progress:0,memberIds};
 markExpeditionMembers(exp,true);state.world.expeditions.push(exp);
 addLog('story',`${region} 원정대 출발`,`${state.year}년 ${state.day}일, ${expeditionMemberNames(exp).join('·')} ${memberIds.length}명이 ${kind==='sea'?'배를 타고':'육로로'} 감나무뜰을 떠났다. 목적지 ${c.name}, 약 ${distanceKm.toLocaleString()}km. 실제 이동시간 ${travelDays}일을 거쳐야 접촉이 성립한다.`,'기록','관찰자는 볼 수 있어도 주민은 직접 도착해야 만날 수 있다.');return true
}
function arriveWorldExpedition(exp){
 if(!exp||exp.contactCompleted)return false;
 exp.contactCompleted=true;exp.phase='return';exp.progress=1;exp.arriveYear=state.year;exp.arriveDay=state.day;
 const c=WORLD_CITY_BY_ID.get(exp.to),targetCountry=exp.country||cityController(c),originCountry=exp.originCountry||'에르단 왕국';
 if(exp.region&&!state.world.knownRegions.includes(exp.region)&&originCountry==='에르단 왕국')state.world.knownRegions.push(exp.region);
 const A=state.world.countries[originCountry],B=state.world.countries[targetCountry];if(A&&targetCountry&&!A.knownCountries.includes(targetCountry))A.knownCountries.push(targetCountry);if(B&&originCountry&&!B.knownCountries.includes(originCountry))B.knownCountries.push(originCountry);
 if((originCountry==='에르단 왕국'||targetCountry==='에르단 왕국')&&targetCountry!=='에르단 왕국'&&!state.world.contactedCountries.includes(targetCountry))state.world.contactedCountries.push(targetCountry);
 if(targetCountry==='에르단 왕국'&&originCountry!=='에르단 왕국'&&!state.world.contactedCountries.includes(originCountry))state.world.contactedCountries.push(originCountry);
 state.world.lastRegionRevealAbsDay=absDay();addWorldRoute(exp.from,exp.to,exp.kind);
 exp.returnStartAbsDay=absDay();exp.returnArrivalAbsDay=absDay()+exp.travelDays;
 const dest=exp.country||exp.region||c?.name||'목적지',members=expeditionMemberNames(exp).join('·')||'원정대';
 addLog('story',`${dest} 도착`,`${members}이 약 ${exp.distanceKm.toLocaleString()}km를 이동해 ${dest}에 도착했다. 주민에게 실제 접촉 정보가 열렸고, 현지 확인 뒤 같은 길로 귀환을 시작했다.`,'기록',`관찰자는 세계지도에서 귀환 중인 원정대도 계속 추적할 수 있다.`);
 return true
}
function finishWorldExpedition(exp){
 exp.active=false;exp.completed=true;exp.phase='complete';exp.progress=0;exp.returnYear=state.year;exp.returnDay=state.day;markExpeditionMembers(exp,false);
 const members=expeditionMemberNames(exp).join('·')||'원정대';
 addLog('good','원정대 귀환',`${members}이 감나무뜰로 돌아왔다. 이동 거리와 접촉 정보가 주민들의 실제 경험으로 남았다.`,'기록',`${exp.distanceKm.toLocaleString()}km 왕복 기록`);
 return true
}
function processWorldExpeditionsDaily(){
 ...
 for(const exp of state.world.expeditions||[]){
   ...
   exp.phase??='outbound';exp.memberIds??=[];
   if(exp.phase==='outbound'){
     exp.progress=clamp((now-exp.startAbsDay)/Math.max(1,exp.arrivalAbsDay-exp.startAbsDay),0,1);
     if(now>=exp.arrivalAbsDay)arriveWorldExpedition(exp)
   }else if(exp.phase==='return'){
     const p=clamp((now-exp.returnStartAbsDay)/Math.max(1,exp.returnArrivalAbsDay-exp.returnStartAbsDay),0,1);
     exp.progress=1-p;if(now>=exp.returnArrivalAbsDay)finishWorldExpedition(exp)
   }
 }
}

function renderExpeditionTracker(){
 const list=$('expeditionTrackerList');if(!list)return;
 const active=(state.world.expeditions||[]).filter(e=>e.active);
 if(!active.length){list.innerHTML='<div class="tracker-empty">현재 이동 중인 원정대가 없습니다.</div>';return}
 list.innerHTML=active.map(exp=>{
   const names=expeditionMemberNames(exp),phase=exp.phase==='return'?'귀환 중':(exp.kind==='sea'?'항해 중':'이동 중'),pct=Math.round((exp.phase==='return'?1-exp.progress:exp.progress)*100);
   return`<article class="tracker-card"><div><b>${exp.kind==='sea'?'⛵':'🥾'} ${exp.country||exp.region}</b><small>${phase} · 구간 ${pct}% · ${expeditionRemainingDays(exp)}일 남음 · ${exp.distanceKm.toLocaleString()}km</small><p>원정자: ${names.join(' · ')||'상세 인원 미기록'}</p></div><button data-track-exp="${exp.id}">세계지도 추적</button>${names.map((n,i)=>`<button class="track-person" data-track-person="${exp.memberIds[i]}" data-track-exp="${exp.id}">👁 ${n}</button>`).join('')}</article>`
 }).join('');
 list.querySelectorAll('[data-track-exp]').forEach(b=>b.onclick=()=>trackExpeditionOnWorldMap(b.dataset.trackExp));
 list.querySelectorAll('[data-track-person]').forEach(b=>b.onclick=()=>trackExpeditionOnWorldMap(b.dataset.trackExp,b.dataset.trackPerson))
}
function openExpeditionTracker(){renderExpeditionTracker();$('expeditionTrackerPanel').classList.remove('hidden')}
function closeExpeditionTracker(){$('expeditionTrackerPanel').classList.add('hidden')}
function trackExpeditionOnWorldMap(expId,personId=null){
 state.world.trackedExpeditionId=expId;state.world.trackedExpeditionPersonId=personId;
 closeExpeditionTracker();worldMapUI.open();worldMapUI.focusExpedition?.(expId)
}

function updateSeaTech(){
 const t=state.world.seaTech,b=state.buildings,years=Math.max(0,state.year+(state.day-1)/365);
 t.sail.p=clamp(Math.max(t.sail.p,years*10.5)+(b.workshop?0.04:0),0,100);t.navigation.p=clamp(Math.max(t.navigation.p,years*10.2)+(state.tech.기록습관.open?.04:0),0,100);t.stores.p=clamp(Math.max(t.stores.p,years*10.1)+(b.storage?.04:0),0,100);
 for(const v of Object.values(t))if(!v.open&&v.p>=100){v.p=100;v.open=true;addLog('good',`${v.label} 준비 완료`,`${v.label}이 장거리 원정에서 반복 사용할 수 있는 기술로 정리됐다.`,'기록','바다로 나가기 전에 돌아올 방법부터 준비했다.')}
}
...
function updateWorldEngine(){
 ...
 const explorerCount=state.residents.filter(r=>r.brain?.action==='explore').length;state.world.landProgress=clamp(state.world.landProgress+.012+explorerCount*.015+avgSkill('채집')*.0002,0,9999);
 if(state.world.seaTech.sail.open&&state.world.seaTech.navigation.open&&state.world.seaTech.stores.open)state.world.seaProgress=clamp(state.world.seaProgress+.035+explorerCount*.012,0,9999);
 // Canon: no resident-to-resident interregional contact before the first long-distance voyage around year 10.
 if(state.year>=10&&!state.world.firstSeaExpedition&&!state.world.expeditions.some(e=>e.active)){
  state.world.seaTech.sail.open=state.world.seaTech.navigation.open=state.world.seaTech.stores.open=true;state.world.seaTech.sail.p=state.world.seaTech.navigation.p=state.world.seaTech.stores.p=100;
  if(scheduleWorldExpedition('네레이아 해권','sea')){state.world.firstSeaExpedition=true;addLog('story','세계력 10년 · 첫 장거리 항해',`동물이 없던 세계에서 인간은 수송과 노동을 기계로 해결해 왔다. 돛·항법·저장기술을 갖춘 감나무뜰의 첫 장거리 원정대가 바다 너머 인간을 찾기 위해 출항했다.`,'기록','세계는 감나무뜰만의 것이 아니었다.')}
 }
 // After the first contact, later expeditions gradually connect the other six distant human regions.
 if(state.world.firstSeaExpedition&&state.year>=12&&!state.world.expeditions.some(e=>e.active)){
  const unknown=INITIAL_REGION_COMMUNITIES.filter(s=>s.country!=='에르단 왕국'&&!state.world.contactedCountries.includes(s.country));
  if(unknown.length&&absDay()-(state.world.lastRegionRevealAbsDay||-999)>180){const s=unknown.sort((a,b)=>cityDistanceKm('L001',a.cityId)-cityDistanceKm('L001',b.cityId))[0];scheduleWorldExpedition(s.region,/해권|군도|부유/.test(s.region)?'sea':'land')}
 }
 syncWorldCityTimeline()
}

function annualWorldPopulation(){
 annualCountryPopulationSimulation();expireTreaties();maybeTradeAndTreaties();simulateExternalWarsAnnual();simulatePoliticsAnnual()
}
...
function tryMilestones(){
 ...
 if(buildReady()&&state.bio?.livestock&&state.tech.사육기초.open&&!b.pen&&state.resources.wood>=15&&state.resources.stone>=3&&state.resources.labor>=10){spend({wood:15,stone:3,labor:10});b.pen=1;state.civ.builds++;markBuilt();syncVillageVisuals(true);addLog('good','첫 사육 울타리','동물이 머물 수 있지만 완전히 갇히지는 않는 생활 울타리가 생겼다.','솔','서로 놀라지 않게 하는 거리여야 해.')}
...
function advanceDay(){
 ...
 if(state.day>365){
   ...
   guarded('생명공학연대',()=>updateBioScienceAnnual());
 ...
 guarded('계절·날씨',()=>{updateClimateDaily(false);if(['봄비','소나기','가을비','폭우'].includes(state.weather))gain({water:state.weather==='폭우'?.9:.28});if(state.weather==='폭설')state.resources.labor=Math.max(0,state.resources.labor-.18)});
...
function currentMiniPlaces(){
 const a=[['감나무뜰 중심',LOC.center],['밭',LOC.field],['강가',LOC.river],['숲',LOC.forest],['채석지',LOC.stone],['작업장',LOC.workshop],['약초지',LOC.herbs],['회의터',LOC.meeting]];
 if(state.bio?.livestock&&state.buildings.pen)a.push(['인공생물 사육구역',LOC.pen]);
 return a
}
function localPlaceName(x,z){
 let best='분지 외곽',bd=Infinity;
 for(const[n,p]of currentMiniPlaces()){const d=Math.hypot(x-p.x,z-p.z);if(d<bd){bd=d;best=n}}
...
const MINI_BIOME_COLORS={
 meadow:'#96a176',forest:'#83956f',lakewood:'#82956f',
 hills:'#858d70',grassland:'#909a70',wetland:'#7f947d'
};
...
function drawMiniStreamingTerrain(){
 ...
 for(let cz=minCZ;cz<=maxCZ;cz++)for(let cx=minCX;cx<=maxCX;cx++){
   ...
   if(hasPond){
     ...
     if(riverDistanceAt(wx,wz)<=RIVER_BANK_WIDTH+18)continue;
   ...
   for(let i=0;i<count;i++){
     ...
     if(!miniVisible(q,4)||riverDistanceAt(wx,wz)<RIVER_BANK_WIDTH*.62)continue;
     const typeSeed=chunkSeed(cx,cz,i*4+2);
     let type='rock';
     if(biome==='forest'||biome==='lakewood')type=typeSeed<.48?'tree':typeSeed<.75?'bush':'rock';
...
function drawMiniVillageOverlay(){
 ...
 // Canonical river: same riverXAtZ() coordinates as the actual streamed 3D water.
 const z0=c.z-range*1.22,z1=c.z+range*1.22,steps=28;
 mctx.save();mctx.strokeStyle='#68a1b2';mctx.lineCap='round';mctx.lineJoin='round';
 mctx.lineWidth=clamp(RIVER_WIDTH/(range*2)*mini.width,5,18);mctx.beginPath();
 for(let i=0;i<=steps;i++){
   const z=z0+(z1-z0)*(i/steps),q=worldToMini(riverXAtZ(z),z);
   if(i===0)mctx.moveTo(q.x,q.y);else mctx.lineTo(q.x,q.y)
 }
 mctx.stroke();mctx.restore();

 if(!nearVillage)return;
 // Roads/field exist only around the original settlement.
...
function drawMini(){
 ...
 for(let i=0;i<(state.buildings.house||0);i++){
   const s=houseSpot(i),q=worldToMini(s[0],s[1]);if(!miniVisible(q))continue;
...
function renderPeopleList(){
 $('peopleList').innerHTML=state.residents.map(r=>{
   const p=personMap.get(r.id),exp=(state.world.expeditions||[]).find(e=>e.active&&e.id===r.travelExpeditionId),task=exp?`원정 중 · ${exp.country||exp.region}`:(p?.userData.task||ACTION_META[r.brain?.action]?.label||'생활 중'),best=Object.entries(r.skills).sort((a,b)=>b[1]-a[1])[0];
   return`<div class="card resident-row"><div class="face" style="background-image:url('${portraitForResident(r)}')"></div><div><b>${r.name} · ${r.job}${exp?' · 🧭 원정':''}</b><br><small>${r.id} · ${r.age}세 · ${task}</small><div class="bar"><i style="width:${r.bloom}%"></i></div><small>“${r.brain?.thought||'주변을 살피는 중'}”</small><br><small>${best[0]} ${Math.round(best[1])} · 개화 ${r.bloom.toFixed(1)}%</small></div><button class="focus-person" data-focus="${r.id}">보기</button></div>`
 ...
 document.querySelectorAll('[data-focus]').forEach(b=>b.onclick=()=>{const r=state.residents.find(x=>x.id===b.dataset.focus),exp=(state.world.expeditions||[]).find(e=>e.active&&e.id===r?.travelExpeditionId);if(exp){$('sheet').classList.add('hidden');trackExpeditionOnWorldMap(exp.id,r.id);return}if(r&&(r.homeCity||'L001')!=='L001'){$('sheet').classList.add('hidden');observerWorldLocation();worldMapUI.open();worldMapUI.focusCity?.(r.homeCity);showEvent(`👁 ${r.name} · ${WORLD_CITY_BY_ID.get(r.homeCity)?.name||r.homeCity}`);return}followId=b.dataset.focus;$('followSelect').value=followId;setCameraMode('follow');renderBrainPanel(r);$('sheet').classList.add('hidden')})
}
function renderWorld(){
 const b=state.buildings,t=state.tech,children=state.residents.filter(r=>r.age<16).length,active=activeCountryNames().map(n=>[n,state.world.countries[n]]).sort((a,b)=>b[1].population-a[1].population);
 $('worldCards').innerHTML=`<div class="card"><h3>${state.civ.levelName} · 감나무뜰</h3><p>정착권 실제 인구 ${localSettlementPopulation().toLocaleString()}명 · 화면 상세추적 ${state.residents.filter(r=>(r.homeCity||'L001')==='L001').length}명 · 세계 실제 인구 ${state.worldPopulation.toLocaleString()}명</p><span class="tag">주택 ${b.house}</span><span class="tag">밭 ${b.field}</span><span class="tag">저장고 ${b.storage}</span><span class="tag">작업장 ${b.workshop}</span></div>
 <div class="card"><h3>인구·분가 원칙</h3><p>0년에는 감나무뜰 30명과 다른 7개 권역 270명, 총 300명뿐이다. 출생·사망으로 인구가 먼저 늘고, 약 1,000명당 하나의 새 도시 슬롯을 가족 단위 분가로 개척한다. 이주는 세계 총인구를 늘리지 않는다.</p><span class="tag">아이 ${children}</span><span class="tag">출생 ${state.demography.births}</span><span class="tag">형성 도시 ${state.world.foundedCities.length}/126</span></div>
 <div class="card"><h3>복실이 · 생명과학의 출발점</h3><p>초기 세계의 비인간 동물은 복실이 하나뿐이다. 복실이를 복제한 것이 아니라 털·침·자연 탈락 세포와 행동을 비침습적으로 연구하면서 인공생명과학이 시작된다.</p><span class="tag">${bioStageLabel()}</span><span class="tag">생명연구 ${Math.floor(state.bio?.score||0)}</span><span class="tag">이명자 · 프롤로그 이전 현실에서 사망</span></div>
 <div class="card"><h3>동물이 없는 초기 세계</h3><p>${state.bio?.livestock?'인공가축이 등장한 뒤라 일부 사육생물이 존재한다.':'새·물고기·벌레·야생동물·가축은 존재하지 않는다. 인간과 식물, 복실이뿐이다.'}</p><span class="tag">야생동물 ${animals.filter(a=>!a.userData.domestic).length}</span><span class="tag">인공 사육생물 ${animals.filter(a=>a.userData.domestic).length}</span></div>
 <div class="card"><h3>8개 권역의 인간</h3><p>감나무뜰 30명, 다른 일곱 권역 270명. 주민들은 다른 권역의 인간을 처음부터 알지 못하며 세계력 10년 전후 실제 장거리 원정으로 접촉한다.</p>${active.map(([n,c])=>`<span class="tag">${c.region} · ${c.population.toLocaleString()}명 · ${c.specialization||REGION_KNOWLEDGE_FOCUS[c.region]||'생활기술'}</span>`).join('')}</div>
 <div class="card"><h3>기계 → 생명공학</h3><p>사람이 너무 적어 소·말이 맡았던 노동까지 인간이 감당해야 한다. 그래서 도르래·수차·풍력·운반기계가 빠르게 발전하고, 그 다음에는 부족한 노동력을 보충하기 위한 인공생명이 발전한다.</p><span class="tag">생활기술 ${countOpenTech()}개</span><span class="tag">${state.world.firstSeaExpedition?'첫 장거리 항해 시작':'첫 장거리 항해 준비'}</span></div>
 <div class="card"><h3>현재 시대 · ${bioStageLabel()}</h3><p>20년대 인공동물, 30년대 목적형 생물, 40년대 군사 생명공학, 55년 최초 오크 조우라는 정사 경계가 적용된다.</p><span class="tag">몬스터 시대 ${state.year>=55?'개방':'55년 잠김'}</span></div>`
}
...
function renderMonsterTargetHud(){
...
function renderNovel(){$('novelPreview').textContent=novelText($('novelStyle').value)}function updatePlayerHud(){
 const p=state.player,aw=p.awakened,s=playerCombatStats();
 $('playerName').textContent=aw?`나 · LV.${p.level} · 전투력 ${s.attack+s.defense}`:'나 · 관찰자';
 let status;
 if(!aw)status=`마을을 걷는 중 · 실험체 출현까지 ${Math.max(0,55-state.year)}년`;
 else if(p.dead)status=`쓰러짐 · 부활 대기 · 사망 ${p.deaths||0}회`;
 else if(selectedMonster&&!selectedMonster.userData.dead)status=`${selectedMonster.userData.code} ${selectedMonster.userData.name} · 공격 ${s.attack} / 방어 ${s.defense}`;
 else status=`EXP ${p.exp}/${p.nextExp} · 은화 ${state.rpg.silver} · 장비 ${equipmentItems().length}/3 · 스킬P ${state.rpg.skillPoints||0}`;
 ...
 $('playerHpBar').style.width=`${clamp(p.hp/s.maxHp*100,0,100)}%`;
...
function renderUI(){
 updatePlayerHud();$('worldDate').textContent=`세계력 ${stamp()}`;$('worldSub').textContent=`${season()} · ${state.weather} · ${state.civ.levelName} · 정착권 ${localSettlementPopulation().toLocaleString()}명 · 상세추적 ${state.residents.length}명`;
...
function showEvent(t){const e=$('eventPop');if(!e)return;e.title=String(t);e.textContent='● '+String(t).slice(0,68);e.classList.add('show');clearTimeout(showEvent.t);showEvent.t=setTimeout(()=>e.classList.remove('show'),1800)}
...
function regionStyle(name){
 return{
  ...
  '실바리온 수림권':{ground:0x829572,water:0x5a91a0,kind:'forest'},
...
function createRegionObserver(){
 const g=new THREE.Group(),skin=new THREE.MeshStandardMaterial({color:0xd3a07c,roughness:.9}),cloth=new THREE.MeshStandardMaterial({color:0x43584f,roughness:1}),dark=new THREE.MeshStandardMaterial({color:0x303735,roughness:1});
 const body=new THREE.Mesh(new THREE.CylinderGeometry(.25,.30,.72,8),cloth);body.position.y=1.18;g.add(body);const head=new THREE.Mesh(new THREE.SphereGeometry(.20,10,8),skin);head.position.y=1.72;g.add(head);
 for(const x of[-.13,.13]){const leg=new THREE.Mesh(new THREE.CylinderGeometry(.05,.06,.65,6),dark);leg.position.set(x,.67,0);g.add(leg)}
 const ring=new THREE.Mesh(new THREE.RingGeometry(.42,.52,24),new THREE.MeshBasicMaterial({color:0xf0d47c,transparent:true,opacity:.8,side:THREE.DoubleSide}));ring.rotation.x=-Math.PI/2;ring.position.y=.02;g.add(ring);
 const marker=makeFloatingNameSprite('▼ 나 · 관찰자');marker.position.set(0,2.4,0);marker.scale.set(2.5,.58,1);g.add(marker);
 const transport=new THREE.Group(),rv=new THREE.Mesh(new THREE.BoxGeometry(1.65,.38,.82),new THREE.MeshStandardMaterial({color:0x405c6b,roughness:.55}));rv.position.y=.45;transport.add(rv);const pl=new THREE.Mesh(new THREE.BoxGeometry(2.3,.10,.72),new THREE.MeshStandardMaterial({color:0xb7c1c4,roughness:.45,metalness:.3}));pl.position.y=.72;transport.add(pl);transport.visible=false;g.add(transport);
 g.scale.setScalar(REGION_HUMAN_SCALE);g.userData={observer:true,target:new THREE.Vector3(),moving:false,transportVisual:transport};return g
}
...
function regionTree(x,z,s=1){
 const g=new THREE.Group(),tr=new THREE.Mesh(new THREE.CylinderGeometry(.14*s,.20*s,1.45*s,6),new THREE.MeshStandardMaterial({color:0x604733,roughness:1})),cr=new THREE.Mesh(new THREE.ConeGeometry(.68*s,1.7*s,7),new THREE.MeshStandardMaterial({color:0x5f835d,roughness:1,transparent:true,opacity:.70}));
 tr.position.y=.72*s;cr.position.y=1.72*s;g.add(tr,cr);g.position.set(x,0,z);regionViewGroup.add(g)
}
...
function decorateGrowingSettlement(country,city,x,z){
 const owner=cityController(city),c=state.world.countries[owner||country];if(!c)return;
 const pop=Math.max(1,cityPopulationCount(city.id,owner)),founded=state.world.foundedCities.includes(city.id),capital=c.capitalId===city.id;
 if(!founded)return;
 const houses=pop<=5?2:pop<=10?3:pop<=20?5:pop<=40?7:clamp(Math.ceil(Math.sqrt(pop)*.85),8,32);
 const woodMat=new THREE.MeshStandardMaterial({color:0x8f714f,roughness:1}),roofMat=new THREE.MeshStandardMaterial({color:0x554334,roughness:1});
 const cols=Math.ceil(Math.sqrt(houses)),rows=Math.ceil(houses/cols);
 for(let i=0;i<houses;i++){
   const row=Math.floor(i/cols),col=i%cols,h=new THREE.Group();
   const base=new THREE.Mesh(new THREE.BoxGeometry(2.3,1.45,1.95),woodMat);base.position.y=.73;
   const roof=new THREE.Mesh(new THREE.ConeGeometry(1.7,.95,4),roofMat);roof.position.y=1.88;roof.rotation.y=Math.PI/4;
   const door=new THREE.Mesh(new THREE.BoxGeometry(.42,.72,.08),new THREE.MeshStandardMaterial({color:0x38281e,roughness:1}));door.position.set(0,.46,1.0);
   h.add(base,roof,door);h.position.set(x+(col-(cols-1)/2)*3.9,0,z+(row-(rows-1)/2)*4);regionViewGroup.add(h)
 }
 const plaza=new THREE.Mesh(new THREE.CylinderGeometry(2.0,2.0,.07,16),new THREE.MeshStandardMaterial({color:0x9a845f,roughness:1}));plaza.position.set(x,.04,z+5);regionViewGroup.add(plaza);
 const fire=new THREE.Mesh(new THREE.ConeGeometry(.28,.64,7),new THREE.MeshStandardMaterial({color:0xe19a52,emissive:0x7c2d11,emissiveIntensity:.4}));fire.position.set(x,.35,z+5);regionViewGroup.add(fire);
 const roadMat=new THREE.MeshStandardMaterial({color:0x947c58,roughness:1});
 const road1=new THREE.Mesh(new THREE.BoxGeometry(12,.035,.75),roadMat);road1.position.set(x,.02,z);regionViewGroup.add(road1);
 const road2=new THREE.Mesh(new THREE.BoxGeometry(.75,.035,12),roadMat);road2.position.set(x,.02,z);regionViewGroup.add(road2);
 if(/해권|군도/.test(c.region)){const pier=new THREE.Mesh(new THREE.BoxGeometry(1.15,.16,7),new THREE.MeshStandardMaterial({color:0x65492f,roughness:1}));pier.position.set(x+6,.1,z);regionViewGroup.add(pier)}
 else if(/산맥|화산/.test(c.region)){const mine=new THREE.Mesh(new THREE.TorusGeometry(1.0,.22,7,14,Math.PI),new THREE.MeshStandardMaterial({color:0x464542,roughness:1}));mine.position.set(x-6,1,z);mine.rotation.z=Math.PI;regionViewGroup.add(mine)}
 else if(/아르케아|대초원|수림/.test(c.region)){for(let i=0;i<3;i++){const f=new THREE.Mesh(new THREE.BoxGeometry(3.3,.04,2.0),new THREE.MeshStandardMaterial({color:0x765b39,roughness:1}));f.position.set(x+6,.03,z-3+i*2.4);regionViewGroup.add(f)}}
 else if(/사막/.test(c.region)){const well=new THREE.Mesh(new THREE.CylinderGeometry(1.1,1.1,.42,12),new THREE.MeshStandardMaterial({color:0x876e53,roughness:1}));well.position.set(x+5,.22,z+4);regionViewGroup.add(well)}
 if(capital){
   const hall=new THREE.Group(),b=new THREE.Mesh(new THREE.BoxGeometry(2.7,1.65,2.0),new THREE.MeshStandardMaterial({color:0x84735d,roughness:1})),r=new THREE.Mesh(new THREE.ConeGeometry(1.95,1,4),roofMat);
   b.position.y=.83;r.position.y=2.08;r.rotation.y=Math.PI/4;hall.add(b,r);hall.position.set(x-5,0,z-4.5);regionViewGroup.add(hall)
 }
 const stage=pop<=5?'작은 야영지':pop<=10?'초기 취락':pop<=20?'작은 촌락':pop<=40?'성장 촌락':c.stage;
 regionLabel(`${city.name} · ${stage} · ${pop}명`,x,z+8.5,3.1)
}
function populateRegionObservers(name,sx,sz){
 ...
 for(const city of cities){const owner=cityController(city),pop=Math.max(1,cityPopulationCount(city.id,owner)),quota=clamp(Math.ceil(pop/3),1,10);
   for(let i=0;i<quota&&made<maxVisible;i++,made++){const p={id:`VIS-${city.id}-${i}`},spread=5+Math.min(9,Math.sqrt(pop)*.7),x=sx(city.x)+(stableHash01(p.id+'x')-.5)*spread,z=sz(city.y)+(stableHash01(p.id+'z')-.5)*spread;makeTinyCitizen(p,x,z)}
 }
}
...
function buildRegional3DMap(name){
 ...
 // Main terrain or sea.
 const planeMat=new THREE.MeshStandardMaterial({color:style.kind==='islands'||style.kind==='volcanic'?style.water:seasonTerrainHex(style.ground),roughness:1});
 ...
 cities.filter(city=>state.world.foundedCities.includes(city.id)).forEach((city,i)=>{const owner=cityController(city);regionCityMarker(city,sx(city.x),sz(city.y),state.world.countries[owner]?.capitalId===city.id);const cc=state.world.countries[owner];if(cc&&cc.active!==false)decorateGrowingSettlement(owner,city,sx(city.x),sz(city.y))});
...
function observerSpawnFor(country,cityId){
 ...
 const founded=regionMapTransform.cities.filter(c=>state.world.foundedCities.includes(c.id)&&(!country||cityController(c)===country));
 if(!city||city.region!==regionViewName||!state.world.foundedCities.includes(city.id))city=founded[0]||regionMapTransform.cities.find(c=>state.world.foundedCities.includes(c.id))||regionMapTransform.cities[0];
 if(!city)return new THREE.Vector3(0,0,0);
 return new THREE.Vector3(regionMapTransform.sx(city.x)+4.5,0,regionMapTransform.sz(city.y)+6.5)
}
...
function updateRegionObserver(dt,now){
 ...
 if(d){
   const sp=6.3*observerTransportMultiplier();regionObserver.position.addScaledVector(d,sp*dt);
...
function observerWorldLocation(){
 ...
 if(regionViewActive&&regionObserver&&regionMapTransform){
   const p=regionObserver.position,rm=regionMapTransform,worldX=(p.x/170)*Math.max(1,rm.maxX-rm.minX)+(rm.minX+rm.maxX)/2,worldY=(p.z/120)*Math.max(1,rm.maxY-rm.minY)+(rm.minY+rm.maxY)/2,city=nearestRegionCity(p.x,p.z);
   state.world.observerLocation={mode:'region',region:regionViewName,country:regionVisitCountry||cityController(city)||city?.country||null,cityId:city?.id||regionVisitCityId||null,label:city?.name||regionViewName,worldX,worldY,localX:p.x,localZ:p.z}
 }else{
   const start=WORLD_CITY_BY_ID.get(WORLD_DATA.meta.startCityId),px=player?.position.x??state.player.x??0,pz=player?.position.z??state.player.z??0;
   // visual offset is intentionally magnified on the planetary map; exact metres are shown in the badge.
   state.world.observerLocation={mode:'laen',region:'아르케아 중앙대륙',country:'에르단 왕국',cityId:start?.id||'L001',label:'감나무뜰',worldX:(start?.x||0)+clamp(px*.12,-38,38),worldY:(start?.y||0)+clamp(pz*.12,-38,38),localX:px,localZ:pz}
 }
...
function enterRegionView(name,opts={}){
 ...
 camMode='follow';autoFocus.copy(regionObserver.position);yaw=.72;pitch=.70;distance=30;resizeWorld(true);updatePlayerLocationHud(true);observerWorldLocation();updateTransportVisual()
}
function exitRegionView(){
 ...
 regionVisitCountry=null;regionVisitCityId=null;resetPlayerCamera();resizeWorld(true);updatePlayerLocationHud(true);updateTransportVisual()
}
...
$('expeditionTrackerClose')?.addEventListener('click',closeExpeditionTracker);
...
updateLocalMapExpansion(false);syncVillageVisuals(false);urbanReplan();updateTransportVisual();renderUI();setTimeout(()=>{$('loading').classList.add('hidden');document.documentElement.dataset.civBoot='ready'},450);setInterval(save,5000);
